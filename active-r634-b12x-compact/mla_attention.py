...
from vllm.model_executor.layers.quantization.exl3 import (
    Exl3LinearMethod,
    exl3_mcg8_mla_query_adj,
    exl3_mcg8_mla_value,
    get_exl3_mcg8_mla_packed_weights,
)
...
# Optional SM120-native route for the prefill MLA query BMM through the
# sparkinfer gemm.bmm BF16 specialization. Default-off; fails closed to the
# exact incumbent path (safe op / torch.bmm) on any rejection.
_B12X_QUERY_BF16_BMM_ENABLED = os.getenv("VLLM_B12X_BF16_BMM_QUERY", "0") == "1"
_B12X_A2A_Q_DIRECT_WRITE_ENABLED = (
    os.getenv("VLLM_B12X_A2A_Q_DIRECT_WRITE", "0") == "1"
)
_BF16_BMM_SPEC = {
    "a_dtype": "bfloat16",
    "b_dtype": "bfloat16",
...
_SPARKINFER_GEMM_BMM: object | None = None
_SPARKINFER_GEMM_BMM_MISSING = False
_QUERY_BF16_BMM_PREWARMED: set[tuple[int, int, int, int]] = set()


def _import_sparkinfer_gemm_bmm() -> object | None:
    global _SPARKINFER_GEMM_BMM, _SPARKINFER_GEMM_BMM_MISSING
    if _SPARKINFER_GEMM_BMM is not None:
        return _SPARKINFER_GEMM_BMM
    if _SPARKINFER_GEMM_BMM_MISSING:
        return None
    try:
        import importlib
        ...
        if not all(callable(getattr(gemm, name, None)) for name in required):
            raise ImportError("sparkinfer.gemm.bmm API incomplete")
    except ImportError:
        _SPARKINFER_GEMM_BMM_MISSING = True
        return None
    _SPARKINFER_GEMM_BMM = gemm
    return gemm


def _run_mla_query_bmm_sparkinfer_bf16(
    query: torch.Tensor,
    weight: torch.Tensor,
    output: torch.Tensor,
) -> bool:
    """Route the MLA query BMM to sparkinfer's BF16 gemm.bmm spec.

    Qualified only for BF16 CUDA ``[H,M,K] @ [H,K,N]`` operands with unit
    innermost strides. Any validation error, missing backend, or CUDA-graph
    capture compile-miss falls back to the incumbent path (fail closed).
    """
    if not _B12X_QUERY_BF16_BMM_ENABLED:
        return False
    if (
        query.ndim != 3
        or weight.ndim != 3
        ...
        or not (query.is_cuda and weight.is_cuda and output.is_cuda)
        or query.shape[0] != weight.shape[0]
        or output.shape[0] != weight.shape[0]
        or query.shape[2] != weight.shape[1]
        or output.shape[2] != weight.shape[2]
        or query.shape[1] < 1
        or int(query.stride(-1)) != 1
        or int(weight.stride(-1)) != 1
        or int(output.stride(-1)) != 1
    ):
        return False
    gemm = _import_sparkinfer_gemm_bmm()
    if gemm is None:
        logger.warning_once(
            "VLLM_B12X_BF16_BMM_QUERY=1 but sparkinfer.gemm.bmm is "
            "unavailable; keeping the incumbent MLA query BMM path."
        )
        return False
    try:
        gemm.bmm(query, weight, output, b_major="n", **_BF16_BMM_SPEC)
        return True
    except Exception:
        logger.warning_once(
            "sparkinfer BF16 gemm.bmm rejected the MLA query BMM; falling "
            "back to the incumbent path."
        )
        return False


def _prewarm_b12x_query_bf16_bmm(
    num_heads: int, nope_dim: int, latent_dim: int, device_index: int
) -> int:
    """Compile every BLOCK_M regime of the BF16 query BMM at weight load."""
    gemm = _import_sparkinfer_gemm_bmm()
    if gemm is None:
        return 0
    key = (int(num_heads), int(nope_dim), int(latent_dim), int(device_index))
    if key in _QUERY_BF16_BMM_PREWARMED:
        return 0
    _QUERY_BF16_BMM_PREWARMED.add(key)
    try:
        weight = torch.zeros(
            (int(num_heads), int(nope_dim), int(latent_dim)),
        ...
        logger.warning_once(
            "sparkinfer BF16 gemm.bmm prewarm failed; the query BMM will "
            "fall back to the incumbent path at runtime."
        )
        return 0
...
def _run_mla_query_bmm(
    ...
    *,
    ...
    if _run_mla_query_bmm_sparkinfer_bf16(query, weight, output):
        return
    if (
        use_safe_op
...
def _canonicalize_sparse_mla_kv_cache_dtype(
    ...
    if backend_name == "B12X_MLA_SPARSE" and is_kvarn_mla_cache_dtype(
        kv_cache_dtype
    ):
...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        self._mla_absorb_exl3_candidate = isinstance(
            getattr(kv_b_proj, "quant_method", None), Exl3LinearMethod
        )
        ...
        def _layer_in_ranges(li: int, spec: str) -> bool:
            for _part in spec.split(","):
                if "-" in _part:
                    _lo, _hi = _part.split("-")
                    if int(_lo) <= li <= int(_hi):
                        return True
                elif _part.isdigit() and int(_part) == li:
                    return True
            return False

        # Layer-wise KVarN KV precision: demote selected target layers to
        # k2 (e.g. KVARN_MLA_K2_LAYERS="40-77"). Draft/MTP prefixes never
        # match "layers.N" and keep the base dtype.
        _k2_layers = os.environ.get("KVARN_MLA_K2_LAYERS", "")
        _k4_layers = os.environ.get("KVARN_MLA_K4_LAYERS", "")
        if _k4_layers and kv_cache_dtype == "kvarn_mla_k5_g64":
            _m = re.search(r"layers\.(\d+)", prefix or "")
            if _m and _layer_in_ranges(int(_m.group(1)), _k4_layers):
                kv_cache_dtype = "kvarn_mla_k4_g64"
        if _k2_layers and kv_cache_dtype == "kvarn_mla_k4_g64":
            _m = re.search(r"layers\.(\d+)", prefix or "")
            if _m and _layer_in_ranges(int(_m.group(1)), _k2_layers):
        ...
        self.dcp_project_before_merge = (
            ...
            and not self._mla_absorb_exl3_candidate
    ...
    def _run_exl3_mcg8_query_adj(
        self, q_nope: torch.Tensor, out: torch.Tensor
    ) -> None:
        if getattr(self, "_mla_absorb_source", None) != "exl3_mcg8":
            raise RuntimeError("Packed EXL3 MLA query called without EXL3 source mode.")
        if (
            q_nope.ndim != 3
            or q_nope.shape[0] != 16
            or q_nope.shape[2] != 192
            or not 1 <= q_nope.shape[1] <= 4096
            or q_nope.dtype != torch.bfloat16
            or tuple(out.shape) != (16, q_nope.shape[1], 512)
            or out.dtype != torch.bfloat16
        ):
            raise ValueError(
                "Packed EXL3 MLA query requires BF16 [16, M, 192] -> "
                "BF16 [16, M, 512] with 1 <= M <= 4096; got "
                f"{tuple(q_nope.shape)} -> {tuple(out.shape)}."
            )
        trellis, suh, svh, head0_parity = self._exl3_mcg8_mla_packed
        exl3_mcg8_mla_query_adj(
            q_nope.contiguous(),
            trellis,
            suh,
    ...
    def _run_exl3_mcg8_value(self, x: torch.Tensor, out: torch.Tensor) -> None:
        if getattr(self, "_mla_absorb_source", None) != "exl3_mcg8":
            raise RuntimeError("Packed EXL3 MLA value called without EXL3 source mode.")
        if (
            x.ndim != 3
            or x.shape[0] != 16
            or x.shape[2] != 512
            ...
            or tuple(out.shape) != (x.shape[1], 16, 256)
            or out.dtype != torch.bfloat16
        ):
            raise ValueError(
                "Packed EXL3 MLA value requires BF16 [16, M, 512] -> "
                "BF16 [M, 16, 256] with 1 <= M <= 4096; got "
    ...
    def forward_impl(
        ...
        if num_mqa_tokens > 0:
            ...
            if fused_mqa_q is not None:
                mqa_q = fused_mqa_q
            elif getattr(self, "_mla_absorb_source", None) == "exl3_mcg8":
                N, B, _ = mqa_q_nope.shape
                mqa_ql_nope = mqa_q_nope.new_empty((N, B, self.kv_lora_rank))
                self._run_exl3_mcg8_query_adj(mqa_q_nope, mqa_ql_nope)
            ...
            if self.impl.dcp_world_size > 1:
                ...
                direct_write_used = False
                if (
                    _B12X_A2A_Q_DIRECT_WRITE_ENABLED
                    and isinstance(mqa_q, tuple)
                    and dcp_use_b12x
                    and not workspace_gather_eligible
                    and not ckv_gather_used
                    and not project_before_merge
                ):
                    # F1 (VLLM_B12X_A2A_Q_DIRECT_WRITE): gather the decode
                    # query straight into the borrowed q workspace lane and
                    # fold the noPE/RoPE cat into the persistent staging
                    # slice. forward_mqa's exact_workspace_alias check then
                    # skips the destination copy. Any precondition miss
                    # returns None and the incumbent chain runs unchanged.
                    direct_gather = getattr(
                        self.impl,
                        "dcp_decode_all_gather_query_in_workspace",
                        None,
                    )
                    if callable(direct_gather):
                        gathered_direct = direct_gather(mqa_q)
                        if gathered_direct is not None:
                            mqa_q = gathered_direct
                            direct_write_used = True
                if (
                    not direct_write_used
                    and isinstance(mqa_q, tuple)
                    and not workspace_gather_eligible
                    and not ckv_gather_used
                ):
                ...
                elif dcp_use_b12x and not direct_write_used:
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        packed = get_exl3_mcg8_mla_packed_weights(self.kv_b_proj)
        ...
        if packed is not None:
            ...
            if geometry != (16, 192, 256, 512):
                raise ValueError(
                    "Unsupported EXL3 MCG8 MLA attention geometry: expected "
                    "(heads, query, value, latent)=(16, 192, 256, 512), got "
                    f"{geometry}."
                )
            if act_dtype != torch.bfloat16:
                raise ValueError(
                    "EXL3 MCG8 MLA absorption requires BF16 model activations, "
                    f"got {act_dtype}."
                )
            self._mla_absorb_source = "exl3_mcg8"
            self._exl3_mcg8_mla_packed = packed
            self._use_b12x_absorb_bmm = False
            self.dcp_project_before_merge = False
            for name in (
                "W_UK_T",
                "W_UV",
                "_b12x_absorb_uk_rhs",
        ...
        if _B12X_QUERY_BF16_BMM_ENABLED:
            _prewarm_b12x_query_bf16_bmm(
                self.num_heads,
                self.qk_nope_head_dim,
                self.kv_lora_rank,
    ...
    def _v_up_proj(self, x: torch.Tensor, out: torch.Tensor):
        ...
        if getattr(self, "_mla_absorb_source", None) == "exl3_mcg8":
            self._run_exl3_mcg8_value(x, out)
            return
        if self.is_aiter_triton_fp4_bmm_enabled:
