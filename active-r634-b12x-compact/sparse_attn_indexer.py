...
# Fuse the per-layer query-split indices + topk-scores all-gathers into one
# packed collective. Default-off; bit-identical by construction (pure data
# movement -- see the fused site below).
_FUSED_QS_GATHER_ENABLED = (
    os.getenv("VLLM_B12X_INDEXER_FUSED_QS_GATHER", "0") == "1"
)
        ...
        for chunk in prefill_metadata.chunks:
            ...
            if use_b12x_indexer:
                ...
                active_page_width = min(
                    needed_pages, int(chunk.block_table.shape[1])
                )
                if active_page_width < needed_pages:
                    # The physical-slot emit (DCP1) translates a selected
                    # candidate through this table WITHOUT a width bound, and
                    # the scorer gathers K rows through it; an under-sized
                    # table would read past the row (illegal access / wrong
                    # attention) instead of erroring. Fail closed.
                    raise RuntimeError(
                        "B12X sparse prefill page table is smaller than the "
                        "chunk's K rows: table_width="
                        f"{active_page_width} pages, needed={needed_pages} "
                        f"pages (local_k_rows={local_k_rows}, "
                ...
                if used_owner_merge:
                    continue
                if qs_active:
                    # Fused option: pack the int32 indices and float32 score
                    # bits into one [rows, 2*topk] int32 payload and ship a
                    # single all-gather. Rank-major concat order along dim 0
                    # and the int32 bit views reproduce, byte for byte, what
                    # the two separate all_gathers delivered; every step
                    # below is pure data movement (cat, all_gather, view,
                    # copy_), so the buffer contents are bit-identical to the
                    # two-collective path. One launch replaces two per layer.
                    fused_gather = (
                        _FUSED_QS_GATHER_ENABLED
                        and topk_scores is not None
                        and topk_scores.dtype == torch.float32
                        and topk_indices.dtype == torch.int32
                        and qs_world_size * (qs_row_end - qs_row_start)
                        == chunk.token_end - chunk.token_start
                    )
                    if fused_gather:
                        idx_local = topk_indices.contiguous()
                        sc_local = topk_scores.contiguous()
                        packed_payload = torch.cat(
                            (idx_local, sc_local.view(torch.int32)), dim=1
                        )
                        gathered_packed = qs_group.all_gather(
                            packed_payload, dim=0
                        )
                        gathered_idx = gathered_packed[:, :topk_tokens]
                        gathered_sc = gathered_packed[:, topk_tokens:]
                        idx_dest = topk_indices_buffer[
                            chunk.token_start : chunk.token_end, :topk_tokens
                        ]
                        sc_dest = topk_scores_buffer[
                            chunk.token_start : chunk.token_end, :topk_tokens
                        ].view(torch.int32)
                        idx_dest.copy_(gathered_idx)
                        sc_dest.copy_(gathered_sc)
                    else:
