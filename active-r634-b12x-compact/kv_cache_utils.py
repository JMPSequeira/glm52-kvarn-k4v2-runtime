...
def _get_kvarn_mla_workspace_config(
    ...
    for group in kv_cache_groups:
        ...
        kvarn_specs.extend(
            ...
            and is_kvarn_mla_cache_dtype(spec.cache_dtype_str)
        )
...
def _get_kvarn_mla_num_blocks(
    ...
    def required_memory(num_pages: int) -> int:
        ...
        workspace_bytes = config.workspace_envelope(
            vllm_config, num_pages
        ).total_bytes
    ...
    if override is not None:
        if override < 0:
            raise ValueError(
                f"num_gpu_blocks_override cannot be negative: {override}"
            )
        if override == 0:
            return 0
        required_bytes = required_memory(override)
        if validate_override_memory and required_bytes > available_memory:
            raise ValueError(
...
def _is_lockstep_mla_spec_layout(
    ...
    if not all(isinstance(spec, MLAAttentionSpec) for spec in specs):
        return False
    if not any(
        is_kvarn_mla_cache_dtype(spec.cache_dtype_str) for spec in specs
    ):
        return False
...
def _get_kv_cache_config_packed(
    ...
    if _use_lockstep_mla_allocation(
        ...
        parallel_config.decode_context_parallel_size,
        parallel_config.prefill_context_parallel_size,
    ) or _get_kvarn_mla_workspace_config(kv_cache_groups) is not None:
...
def get_kv_cache_config_from_groups(
    ...
    available_memory: int,
    *,
    validate_override_memory: bool = True,
) -> KVCacheConfig:
    """
...
def _get_kv_cache_groups_uniform_groups(
    ...
    for sm_spec in swa_mla_specs:
        ...
        is_dcp_replicated = all(
            getattr(spec, "dcp_replicated", False)
            for spec in sm_spec.kv_cache_specs.values()
        )
        if not is_dcp_replicated:
