...
def kvarn_mla_workspace_envelope(
    *,
    ...
    if unsupported:
        raise ValueError(
            f"Unsupported KVarN MLA workspace dimensions: {unsupported}"
        )
    invalid = {
        name: value
        for name, value in dimensions.items()
...
def _kvarn_mla_decode_settings() -> tuple[int, bool, bool]:
    ...
    mode = os.environ.get(_MLA_SPEC_EXTEND_MODE_ENV, "auto").strip().lower()
...
# The MLA KVarN KV-cache dtype is self-describing: the string itself carries
# the latent bit width and the variance-normalization tile (group) size, e.g.
# ``kvarn_mla_k4_g64``. It is the SOLE carrier of both — there is no
# environment override for the width. Downstream geometry gates
# (workspace_envelope, the b12x packed-geometry contract) fail closed on any
# width/group combination the kernels do not implement.
KVARN_MLA_CACHE_DTYPE_PATTERN = re.compile(
    r"kvarn_mla_k(?P<bits>2|4|5)_g(?P<group>64|128)"
)


def is_kvarn_mla_cache_dtype(cache_dtype: object) -> bool:
    """True for a self-describing MLA KVarN dtype string ``k{n}_g{m}``."""
    return isinstance(cache_dtype, str) and (
        KVARN_MLA_CACHE_DTYPE_PATTERN.fullmatch(cache_dtype) is not None
    )
...
class KVarNMLAConfig:
    ...
    def workspace_envelope(
        ...
        if geometry not in supported:
            raise ValueError(
                f"Unsupported KVarN MLA workspace geometry {geometry}; "
    ...
    def from_cache_dtype(cls, cache_dtype: str) -> "KVarNMLAConfig":
        """Parse a self-describing dtype ``kvarn_mla_k{n}_g{m}``.

        The string is the only carrier of width and group; nothing is read
        from the environment for either.
        """
        match = (
            KVARN_MLA_CACHE_DTYPE_PATTERN.fullmatch(cache_dtype)
            if isinstance(cache_dtype, str)
            else None
        )
        if match is None:
            raise ValueError(
                "MLA KVarN requires a self-describing kv-cache dtype "
                "'kvarn_mla_k{2|4|5}_g{64|128}' "
                "(e.g. kvarn_mla_k5_g64, kvarn_mla_k4_g64, kvarn_mla_k2_g64); "
                f"got {cache_dtype!r}"
            )
        return cls(
            bits=int(match["bits"]),
