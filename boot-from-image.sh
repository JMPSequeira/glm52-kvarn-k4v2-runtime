...
KVARN_MLA_ALPHA=${KVARN_MLA_ALPHA:-}
KVARN_MLA_DIAG_PACK_DUMP=${KVARN_MLA_DIAG_PACK_DUMP:-}
KVARN_MLA_ORPHAN_RAISE=${KVARN_MLA_ORPHAN_RAISE:-0}
...
KVARN_MLA_DIAG_PACK_ERR=${KVARN_MLA_DIAG_PACK_ERR:-0}
KVARN_MLA_DIAG_REHYDR_DUMP=${KVARN_MLA_DIAG_REHYDR_DUMP:-}
KVARN_MLA_DISABLE_STALE_FILL_GUARD=${KVARN_MLA_DISABLE_STALE_FILL_GUARD:-0}
KVARN_MLA_K2_LAYERS=${KVARN_MLA_K2_LAYERS:-}
KVARN_MLA_K4_LAYERS=${KVARN_MLA_K4_LAYERS:-}
KVARN_MLA_DIAG_LAYER_NORMS=${KVARN_MLA_DIAG_LAYER_NORMS:-}
KVARN_MLA_DIAG_RESTORE_STATE=${KVARN_MLA_DIAG_RESTORE_STATE:-}
...
exec podman run --rm --pull=never --replace \
  ...
  --volume "$SRC/active-r634-kvarn-k4-native/kvarn_mla/io.py:/opt/venv/lib/python3.12/site-packages/sparkinfer/attention/kvarn_mla/io.py" \
  ...
  --env KVARN_MLA_TOPOLOGY_LOG="${KVARN_MLA_TOPOLOGY_LOG:-0}" \
  --env KVARN_MLA_RECORDS_PTR_FIX="${KVARN_MLA_RECORDS_PTR_FIX:-1}" \
  --env KVARN_MLA_SKIP_LEN_GATE="${KVARN_MLA_SKIP_LEN_GATE:-0}" \
  --env KVARN_SCHED_TRACE="${KVARN_SCHED_TRACE:-0}" \
  --env KVARN_FP8_ROPE_RECORD="${KVARN_FP8_ROPE_RECORD:-0}" \
  --env CUDA_LAUNCH_BLOCKING="${CUDA_LAUNCH_BLOCKING:-0}" \
  --env KVARN_READER_TRACE="${KVARN_READER_TRACE:-0}" \
  --env KVARN_FP8_ROPE_DEBUG_CONST="${KVARN_FP8_ROPE_DEBUG_CONST:-0}" \
  --env KVARN_FP8_ROPE_BYPASS="${KVARN_FP8_ROPE_BYPASS:-0}" \
  --env KVARN_FP8_LATENT_CONST="${KVARN_FP8_LATENT_CONST:-0}" \
  ...
  --env KVARN_MLA_DIAG_PACK_ERR="${KVARN_MLA_DIAG_PACK_ERR:-0}" \
  --env KVARN_MLA_DIAG_REHYDR_DUMP="${KVARN_MLA_DIAG_REHYDR_DUMP:-}" \
  --env KVARN_MLA_DISABLE_STALE_FILL_GUARD="${KVARN_MLA_DISABLE_STALE_FILL_GUARD:-0}" \
  --env KVARN_MLA_K2_LAYERS="${KVARN_MLA_K2_LAYERS:-}" \
  --env KVARN_MLA_K4_LAYERS="${KVARN_MLA_K4_LAYERS:-}" \
  --env KVARN_MLA_DIAG_LAYER_NORMS="${KVARN_MLA_DIAG_LAYER_NORMS:-}" \
  --env KVARN_MLA_DIAG_RESTORE_STATE="${KVARN_MLA_DIAG_RESTORE_STATE:-}" \
  --env VLLM_B12X_CKV_POOL_STATE="${VLLM_B12X_CKV_POOL_STATE:-0}" \
