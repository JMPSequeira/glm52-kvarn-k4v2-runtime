...
def _routing(m: int, materialized: int, device: torch.device) -> torch.Tensor:
    ...
    k4_per_row = max(1, TOPK // 4)
    k3_per_row = TOPK - k4_per_row
    for _ in range(m):
        k3 = torch.randperm(materialized, generator=generator)[:k3_per_row]
        k4 = K3_EXPERTS + torch.randperm(materialized, generator=generator)[:k4_per_row]
...
def _serial_state(m, tier, expert_map, props, device, moe_block_size):
...
def _run_serial(state, x, weights, ids):
    ...
    return run_w4a16_moe(
        ...
        route_block_size_m=launch.moe_block_size,
...
def main() -> None:
    global HIDDEN, INTERMEDIATE, TOPK
    global TILE_CONFIG, K3_EXPERTS, K4_EXPERTS, TOTAL_EXPERTS
    ...
    parser.add_argument("--k3-experts", type=int, default=K3_EXPERTS)
    parser.add_argument("--k4-experts", type=int, default=K4_EXPERTS)
    parser.add_argument("--hidden", type=int, default=HIDDEN)
    parser.add_argument("--intermediate", type=int, default=INTERMEDIATE)
    parser.add_argument("--topk", type=int, default=TOPK)
    parser.add_argument("--moe-block-size", type=int, default=8)
    parser.add_argument("--compare-tile-config", type=int, nargs=4)
    parser.add_argument("--compare-block-size", type=int)
    parser.add_argument(
        "--variant",
    ...
    TOPK = int(args.topk)
    if HIDDEN <= 0 or INTERMEDIATE <= 0 or TOPK < 2:
        raise ValueError("--hidden/--intermediate must be positive and --topk >= 2")
    K3_EXPERTS = int(args.k3_experts)
    K4_EXPERTS = int(args.k4_experts)
    if K3_EXPERTS <= 0 or K4_EXPERTS <= 0:
        raise ValueError("--k3-experts and --k4-experts must be positive")
    TOTAL_EXPERTS = K3_EXPERTS + K4_EXPERTS
    moe_block_size = int(args.moe_block_size)
    if moe_block_size not in (8, 16, 32, 64):
        raise ValueError("--moe-block-size must be one of 8, 16, 32, or 64")
    compare_tile_config = (
        None
        if args.compare_tile_config is None
        else tuple(int(value) for value in args.compare_tile_config)
    )
    compare_block_size = (
        moe_block_size
        if args.compare_block_size is None
        else int(args.compare_block_size)
    )
    if compare_block_size not in (8, 16, 32, 64):
        raise ValueError("--compare-block-size must be one of 8, 16, 32, or 64")
    ...
    print(
        ...
        f"tile={TILE_CONFIG} block_m={moe_block_size}"
    )
    for m in args.m:
        ...
        if args.variant in ("both", "serial", "serial-overlap"):
            state0 = (
                tier0,
                *_serial_state(
                    m, tier0, map0, props, device, moe_block_size
                ),
            )
            state1 = (
                tier1,
                *_serial_state(
                    m, tier1, map1, props, device, moe_block_size
                ),
            )
        ...
        if args.variant in ("both", "mixed"):
            route_slots = max_packed_route_slots(
                m * TOPK, moe_block_size, TOTAL_EXPERTS
            )
            ...
            if compare_tile_config is not None:
                compare_route_slots = max_packed_route_slots(
                    m * TOPK, compare_block_size, TOTAL_EXPERTS
                )
                compare_launch = compile_mixed_trellis(
                    size_m=m,
                    hidden_size=HIDDEN,
                    intermediate_size=INTERMEDIATE,
                    tier0_num_experts=K3_EXPERTS,
                    ...
                    top_k=TOPK,
                    max_m_blocks=(
                        compare_route_slots + compare_block_size - 1
                    )
                    // compare_block_size,
                    moe_block_size=compare_block_size,
                    sms=int(props.multi_processor_count),
                    max_shared_mem=int(props.shared_memory_per_block_optin),
                    force_tile_config=compare_tile_config,
                )
                compare_buffers = make_mixed_trellis_buffers(
                    compare_launch,
                    device=device,
                    sms=int(props.multi_processor_count),
                )
                ...
                torch.cuda.synchronize()
                rel = (
                    compare_buffers.output[:m] - mixed_buffers.output[:m]
                ).norm() / mixed_buffers.output[:m].norm().clamp_min(1.0e-12)
                cosine = torch.nn.functional.cosine_similarity(
                    compare_buffers.output[:m].flatten(),
                    mixed_buffers.output[:m].flatten(),
                    dim=0,
                )
                if float(rel) > 4.0e-3 or float(cosine) < 0.99999:
                    raise RuntimeError(
                        "mixed-pair correctness failed: "
                        f"rel={float(rel)} cos={float(cosine)}"
                    )
                print(
                    f"compare_tile={compare_tile_config} "
                    f"compare_block_m={compare_block_size} "
                    f"rel={float(rel):.3e} cos={float(cosine):.8f}"
                )
