...
def _fused_radix_select(
                        ...
                        if Int32(bin) > sub_threshold:
                            ...
                            if Int32(bin) == sub_threshold:
                                if cutlass.const_expr(round_idx == 3):
                                    # Resolve the exact-score boundary by global
                                    # token ID instead of atomic arrival order.
                                    # IDs are unique, so each selected tie owns
                                    # one collision-free output position.
                                    remaining = _smem_ld(lr, Int32(0))
                                    gid = Int32(vin_g[c_idx])
                                    rank = Int32(0)
                                    j = Int32(0)
                                    while j < num_input:
                                        other_idx = (
                                            Int32(s_cand0[j])
                                            if cutlass.const_expr(r_idx_is_0)
                                            else Int32(s_cand1[j])
                                        )
                                        if (
                                            Float32(vin_v[other_idx]) == raw_val
                                        ) & (Int32(vin_g[other_idx]) < gid):
                                            rank += Int32(1)
                                        j += Int32(1)
                                    if rank < remaining:
                                        s_out[
                                            topk_static - remaining + rank
                                        ] = c_idx
                                else:
                                    cand_pos = (
    ...
    if bin_count > Int32(cands):
        ...
        # The shared fallback's final equality fill is atomic because generic
        # tiled callers have no token-ID tensor. Fused selection does: rebuild
        # its output with canonical smaller-ID boundary membership so overflow
        # cannot reintroduce request-to-request races.
        cute.arch.sync_threads()
        overflow_pivot = Uint32(_smem_ld(ni0, Int32(0)))
        if tx == Int32(0):
            _smem_st(ctr, Int32(0), Int32(0))
        cute.arch.sync_threads()
        i = Int32(tx)
        while i < n_total:
            if _convert_to_uint32(Float32(vin_v[i])) > overflow_pivot:
                pos = _smem_xadd(ctr, Int32(0), Int32(1))
                if pos < topk_static:
                    s_out[pos] = i
            i += Int32(_RADIX_THREADS)
        cute.arch.sync_threads()
        greater_count = _smem_ld(ctr, Int32(0))
        i = Int32(tx)
        while i < n_total:
            raw_val = Float32(vin_v[i])
            if _convert_to_uint32(raw_val) == overflow_pivot:
                gid = Int32(vin_g[i])
                rank = Int32(0)
                j = Int32(0)
                while j < n_total:
                    if (Float32(vin_v[j]) == raw_val) & (
                        Int32(vin_g[j]) < gid
                    ):
                        rank += Int32(1)
                    j += Int32(1)
                pos = greater_count + rank
                if pos < topk_static:
                    s_out[pos] = i
            i += Int32(_RADIX_THREADS)
    cute.arch.sync_threads()
...
class SparseNSAFusedIndexerKernel:
    ...
    def kernel(
        ...
        if cutlass.const_expr(not self.merge_in_kernel):
            ...
            tie_value = Float32(0.0)
            other_value = Float32(0.0)
            ...
            if (seq_len > Int32(self.merge_threshold)) & (coop_possible != Int32(0)):
                ...
                if total <= topk_static:
                    ...
                    # Materialize every CTA carry at a fixed CTA-owned slab
                    # offset. The group barrier makes the complete candidate
                    # set visible before exact-pivot membership is ranked.
                    base = cta_in_group * topk_static
                    i = Int32(tx)
                    while i < topk_static:
                        if i < carry_count:
                            pack_v_row[base + i] = Float32(s_c0_values[i])
                            pack_i_row[base + i] = Int32(s_c0_gindex[i])
                        else:
                            pack_v_row[base + i] = Float32(float("-inf"))
                            pack_i_row[base + i] = Int32(-1)
                        i += Int32(_RADIX_THREADS)
                    barrier_phase = _group_barrier(
                        merge_state, group_id, barrier_phase, ctas_pg, tx
                    )
                    greater_total = Int32(
                        merge_state[
                            _state_offset(
                                group_id, Int32(_STATE_OUTPUT_COUNTER)
                            )
                        ]
                    )

                    # Exact-score boundary membership is canonical by smaller
                    # token ID across the whole group. Unlike atomic first-wins,
                    # unique IDs give every selected tie a collision-free slot.
                    i = Int32(tx)
                    while i < carry_count:
                        tie_value = Float32(s_c0_values[i])
                        key = _convert_to_uint32(tie_value)
                        if key == ordered_pivot:
                            gid = Int32(s_c0_gindex[i])
                            rank = Int32(0)
                            j = Int32(0)
                            while j < cap:
                                other_value = Float32(pack_v_row[j])
                                if (
                                    _convert_to_uint32(other_value)
                                    == ordered_pivot
                                ) & (Int32(pack_i_row[j]) < gid):
                                    rank += Int32(1)
                                j += Int32(1)
                            pos = greater_total + rank
                            if pos < topk_static:
                                out_values[group_id, pos] = tie_value
                                out_indices[group_id, pos] = gid
                        i += Int32(_RADIX_THREADS)
...
def _launch_fused(kernel, cute_args, key_tensors, policy):
    ...
    """
    # Every fused variant receives a fresh key because canonical tie membership
    # changes local trims, serial overflow, and cooperative merge code. Keeping
    # any prior cubin would silently retain atomic first-wins selection.
    if kernel.direct_k_score:
        ...
        variant = (
            "fused_indexer_v6_directk64"
            if kernel.coop_co_resident
            else "fused_indexer_v6_directk64_oversub_serial"
        )
    elif kernel.coop_co_resident:
        variant = "fused_indexer_v4_coop"
    else:
        variant = "fused_indexer_v4_oversub_serial"
