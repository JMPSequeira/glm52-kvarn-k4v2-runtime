        ...
        if need_radix:
            ...
            if topk != Int32(0):
                                ...
                                if Int32(bin) > sub_threshold:
                                    ...
                                    if Int32(bin) == sub_threshold:
                                        if cutlass.const_expr(round_idx == 3):
                                            ...
                                            gid = _emit_global_index_virtual(
                                                carry_indices,
                                                output_page_table,
                                                output_page_table_row_stride,
                                                row_start,
                                            ...
                                            while j < num_input:
                                                ...
                                                other_value = _load_value_virtual(
                                                    input_tensor,
                                                    carry_values,
                                                    row_base,
                                                    row_start,
                ...
                if bin_count > Int32(_SMEM_CANDS):
                    ...
                    # Generic fallback fills exact-pivot ties by atomic arrival.
                    # Tiled indexer callers can reconstruct the emitted logical
                    # or physical token ID, so rebuild canonical membership.
                    cute.arch.sync_threads()
                    ...
                    while i < total_len:
                        ...
                        if _convert_to_uint32(value_i) > overflow_pivot:
                            pos = _smem_xadd(ctr, Int32(0), Int32(1))
                            if pos < topk_static:
                                s_out[pos] = i
                        i += Int32(_THREADS_PER_CTA)
                    cute.arch.sync_threads()
                    ...
                    while i < total_len:
                        ...
                        if _convert_to_uint32(value_i) == overflow_pivot:
                            ...
                            while j < total_len:
                                ...
                                gid_j = _emit_global_index_virtual(
                                    carry_indices,
                                    output_page_table,
                                    output_page_table_row_stride,
                                    row_start,
...
def run_tiled_topk(
    *,
    ...
    cache_key = (
        ...
        (
            "tiled_topk_v28_runtime_page_stride",
            topk,
            block_q,
            block_k,
            bool(zero_row_start),
