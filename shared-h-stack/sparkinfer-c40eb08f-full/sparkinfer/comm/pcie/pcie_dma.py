...
class PCIeDmaAllReduce:
    ...
    def _pick_pieces(shard_elems: int, shard_bytes: int) -> int:
        ...
        # Six pieces best saturate large Gen5 PCIe BF16 shards while retaining
        # two for smaller transfers where launch overhead dominates.
        candidates = (
            (override,)
            if 1 <= override <= MAX_PIECES
            else ((6,) if shard_bytes >= 6 << 20 else (2,))
        )
