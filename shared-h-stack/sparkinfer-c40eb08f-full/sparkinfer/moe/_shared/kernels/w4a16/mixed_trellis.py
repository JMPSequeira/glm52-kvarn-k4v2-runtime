...
class W4A16MixedTrellisKernel:
    """One cooperative grid over two, three, or four native Trellis bitrates."""
    ...
    def __init__(
        ...
        *,
        ...
        tier2: W4A16FusedMoeKernel | None = None,
        tier3: W4A16FusedMoeKernel | None = None,
    ):
        if tier2 is None and tier3 is not None:
            raise ValueError("mixed Trellis tier3 requires tier2")
        if tier2 is None:
            active_tiers = (tier0, tier1)
        elif tier3 is None:
            active_tiers = (tier0, tier1, tier2)
        else:
            active_tiers = (tier0, tier1, tier2, tier3)
        kernels = (driver, *active_tiers)
        for name, moe in zip(
            ("driver", "tier0", "tier1", "tier2", "tier3"), kernels, strict=False
        ):
        ...
        if any(tier.num_experts > 256 for tier in active_tiers):
            raise ValueError("tier-local expert ids must fit in eight bits")
        if driver.num_experts != sum(tier.num_experts for tier in active_tiers):
            raise ValueError("driver expert count must equal the sum of all tiers")
        self.driver = driver
        self.tier0 = tier0
        self.tier1 = tier1
        self.tier2 = tier2 if tier2 is not None else tier0
        self.tier3 = tier3 if tier3 is not None else tier1
        self.tier_count = len(active_tiers)
        ...
        self.sms = driver.sms
        self.blocks_per_sm = min(moe.blocks_per_sm for moe in kernels)
        self.shared_words = max(moe.shared_words for moe in kernels)
    ...
    def __cache_key__(self) -> tuple[object, ...]:
        return (
            ...
            getattr(self, "tier_count", 2),
            self.tier2.__cache_key__ if getattr(self, "tier_count", 2) >= 3 else None,
            self.tier3.__cache_key__ if getattr(self, "tier_count", 2) == 4 else None,
    ...
    def _emit_tier_tile(
        ...
        if combined_expert >= Int32(0) and combined_expert < total_experts:
            ...
            if descriptor >= Int32(0):
                ...
                if tier == Int32(0) and local_expert < tier0_num_experts:
                ...
                elif cutlass.const_expr(self.tier_count >= 3):
                    if tier == Int32(2) and local_expert < tier2_num_experts:
                        if cutlass.const_expr(is_fc1):
                            gemm = self.tier2.fc1
                        else:
                            gemm = self.tier2.fc2
                    ...
                    elif tier == Int32(3) and local_expert < tier3_num_experts:
                        if cutlass.const_expr(is_fc1):
                            gemm = self.tier3.fc1
                        else:
                            gemm = self.tier3.fc2
    ...
    def __call__(
        ...
        tier2_storage_experts = tier2_experts
        tier3_storage_experts = tier3_experts
        if cutlass.const_expr(self.tier_count < 3):
            tier2_storage_experts = tier0_experts
        if cutlass.const_expr(self.tier_count < 4):
            tier3_storage_experts = tier1_experts
        ...
        t2_w13 = cute.make_tensor(
            t2_w13_ptr,
            layout=cute.make_layout(
                (
                    tier2_storage_experts
                    * cutlass.Int64(self.hidden_size // 16)
                    * cutlass.Int64(self.driver.fc1_cols // 16)
        ...
        t3_w13 = cute.make_tensor(
            t3_w13_ptr,
            layout=cute.make_layout(
                (
                    tier3_storage_experts
                    * cutlass.Int64(self.hidden_size // 16)
                    * cutlass.Int64(self.driver.fc1_cols // 16)
        ...
        t2_w13_scales = cute.make_tensor(
            t2_w13_scales_ptr,
            layout=cute.make_layout(
                (
                    tier2_storage_experts
        ...
        t3_w13_scales = cute.make_tensor(
            t3_w13_scales_ptr,
            layout=cute.make_layout(
                (
                    tier3_storage_experts
        ...
        t2_w13_global = cute.make_tensor(
            t2_w13_global_ptr,
            layout=cute.make_layout((tier2_storage_experts,), stride=(1,)),
        )
        t2_w2_global = cute.make_tensor(
            t2_w2_global_ptr,
            layout=cute.make_layout((tier2_storage_experts,), stride=(1,)),
        )
        t3_w13_global = cute.make_tensor(
            t3_w13_global_ptr,
            layout=cute.make_layout((tier3_storage_experts,), stride=(1,)),
        )
        t3_w2_global = cute.make_tensor(
            t3_w2_global_ptr,
            layout=cute.make_layout((tier3_storage_experts,), stride=(1,)),
        )
...
def compile_mixed_trellis(
    *,
    ...
    tier0_bits: int = 3,
    tier1_bits: int = 4,
    tier2_num_experts: int = 0,
    tier3_num_experts: int = 0,
    tier2_bits: int = 5,
    tier3_bits: int = 6,
    ...
    if int(tier2_num_experts) == 0 and int(tier3_num_experts) != 0:
        raise ValueError("mixed Trellis tier3 requires a nonempty tier2")
    tier_count = (
        2 if int(tier2_num_experts) == 0 else 3 if int(tier3_num_experts) == 0 else 4
    )
    expert_counts = (
        int(tier0_num_experts),
        int(tier1_num_experts),
        int(tier2_num_experts),
        int(tier3_num_experts),
    )
    if any(count <= 0 for count in expert_counts[:tier_count]):
        raise ValueError("mixed Trellis active tiers must be nonempty")
    all_bits = (
        int(tier0_bits),
        int(tier1_bits),
        int(tier2_bits),
    ...
    active_bits = all_bits[:tier_count]
    if len(set(active_bits)) != tier_count:
        raise ValueError("mixed Trellis tiers require distinct bitrates")
    ...
    total_experts = sum(expert_counts[:tier_count])
    ...
    kernel = W4A16MixedTrellisKernel(
        ...
        tier0=make_kernel(expert_counts[0], int(tier0_bits)),
        tier1=make_kernel(expert_counts[1], int(tier1_bits)),
        tier2=(
            make_kernel(expert_counts[2], int(tier2_bits)) if tier_count >= 3 else None
        ),
        tier3=(
            make_kernel(expert_counts[3], int(tier3_bits)) if tier_count == 4 else None
        ),
    )
    ...
    if cached is not None:
        ...
        return replace(
            ...
            tier2_num_experts=int(tier2_num_experts),
            tier3_num_experts=int(tier3_num_experts),
            tier2_bits=int(tier2_bits),
            tier3_bits=int(tier3_bits),
    ...
    compile_args = (
        ...
        *tier_args(),
        ...
        Int32(tier2_num_experts),
        Int32(tier3_num_experts),
        1,
        1,
        current_cuda_stream(),
    )
...
def make_mixed_trellis_buffers(
    ...
    *,
    ...
    total_experts = (
        launch.tier0_num_experts
        + launch.tier1_num_experts
        + launch.tier2_num_experts
        + launch.tier3_num_experts
    )
...
def build_tiered_maps(
    ...
    *,
    ...
    """Build immutable route and descriptor maps for two expert tiers."""

    return build_multitier_maps((tier0_global_ids, tier1_global_ids), device=device)


def build_multitier_maps(
    tier_global_ids: Sequence[Sequence[int]],
    *,
    device: torch.device,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Build immutable route maps for two, three, or four expert tiers."""

    tiers = tuple(
        tuple(int(expert_id) for expert_id in expert_ids)
        for expert_ids in tier_global_ids
    )
    if len(tiers) not in (2, 3, 4):
        raise ValueError("mixed Trellis requires two, three, or four expert tiers")
    if any(not tier or len(tier) > 256 for tier in tiers):
        raise ValueError("each mixed Trellis tier requires 1..256 experts")
    flattened = tuple(expert_id for tier in tiers for expert_id in tier)
    total = len(flattened)
    if sorted(flattened) != list(range(total)):
        raise ValueError(
    ...
    for tier_index, tier in enumerate(tiers):
        for local_id, global_id in enumerate(tier):
            global_to_combined_host[global_id] = combined_id
            descriptor_host.append((tier_index << 8) | local_id)
            combined_id += 1
...
def combine_trellis_rotations(
    *tiers: MixedTrellisTier,
) -> MixedTrellisRotations:
    """Materialize one tier-ordered table set once during model preparation."""
    if len(tiers) not in (2, 3, 4):
        raise ValueError("mixed Trellis rotations require two, three, or four tiers")
...
def run_mixed_trellis(
    ...
    tier2: MixedTrellisTier | None = None,
    tier3: MixedTrellisTier | None = None,
) -> torch.Tensor:
    m = int(x.shape[0])
    if m <= 0:
    ...
    if launch.tier_count == 4:
        if tier2 is None or tier3 is None:
            raise ValueError("four-tier launch requires tier2 and tier3 storage")
        tiers = (tier0, tier1, tier2, tier3)
    elif launch.tier_count == 3:
        if tier2 is None or tier3 is not None:
            raise ValueError("three-tier launch requires only tier2 storage")
        tiers = (tier0, tier1, tier2)
    else:
        if tier2 is not None or tier3 is not None:
            raise ValueError("two-tier launch does not accept tier2 or tier3")
        tiers = (tier0, tier1)
    expected_counts = (
        launch.tier0_num_experts,
        launch.tier1_num_experts,
    ...
    for index, tier in enumerate(tiers):
        name = f"tier{index}"
        expected_experts = expected_counts[index]
        ...
        _validate_mixed_trellis_tier_storage(
            ...
            bits=expected_bits[index],
            hidden_size=launch.hidden_size,
            intermediate_size=launch.intermediate_size,
            device=x.device,
        )
    total_experts = sum(expected_counts[: launch.tier_count])
    for name, mapping in (
    ...
    if launch.tier_count == 4:
        ...
        runtime_tiers = (*tiers, tier0)
    else:
        runtime_tiers = (tier0, tier1, tier0, tier1)

    def tier_pointer_args(tier: MixedTrellisTier) -> tuple[object, ...]:
        return (
            make_ptr(
                cutlass.Int32,
                tier.w13.data_ptr(),
    ...
    launch.compiled(
        ...
        *tier_pointer_args(runtime_tiers[0]),
        *tier_pointer_args(runtime_tiers[1]),
        *tier_pointer_args(runtime_tiers[2]),
        *tier_pointer_args(runtime_tiers[3]),
        buffers.fc1.view(-1),
        ...
        Int32(launch.tier2_num_experts),
        Int32(launch.tier3_num_experts),
        m,
