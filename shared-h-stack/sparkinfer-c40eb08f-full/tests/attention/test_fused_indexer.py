...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required for fused indexer")
def test_fused_indexer_paged_boundary_ties_choose_smaller_ids_deterministically():
    device = torch.device("cuda")
    rows, heads, topk, seqlen = 1, 32, 2048, 8192
    q_fp8, weights, k_fp8, k_scales, page_table, seqlens = _build_case(
        rows, heads, seqlen, topk, seed=19, device=device
    )

    # Repeat each K/scaling record three times. K is not divisible by three, so
    # the selection boundary necessarily cuts one exact-score tie.
    source = (torch.arange(seqlen, device=device) // 3) * 3
    flat_k = k_fp8.reshape(-1, k_fp8.shape[-1])
    flat_scales = k_scales.reshape(-1)
    flat_k[:seqlen].copy_(flat_k[source])
    flat_scales[:seqlen].copy_(flat_scales[source])

    _, _, scores = _golden_topk(
        q_fp8,
        weights,
        k_fp8,
        k_scales,
    ...
    score_row = scores[0].cpu().tolist()
    expected = torch.tensor(
        sorted(range(seqlen), key=lambda token_id: (-score_row[token_id], token_id))[
            :topk
        ],
        dtype=torch.int32,
    ).sort().values
    ...
    for _ in range(25):
        indices, _ = run_fused_paged_indexer(
            q_bytes=q_fp8.view(torch.uint8),
            weights=weights,
            k_quant_bytes=k_fp8.view(torch.uint8).contiguous(),
            k_scales=k_scales,
        ...
        torch.cuda.synchronize(device)
        assert torch.equal(indices[0].cpu().sort().values, expected)
