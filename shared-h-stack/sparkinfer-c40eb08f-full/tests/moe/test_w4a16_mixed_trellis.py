...
def _mixed4_cache_key(counts: tuple[int, int, int, int]) -> tuple[object, ...]:
    kernel = object.__new__(W4A16MixedTrellisKernel)
    kernel.driver = SimpleNamespace(__cache_key__=("driver",))
    for index, bits in enumerate((3, 4, 5, 6)):
        setattr(
            kernel,
            f"tier{index}",
            SimpleNamespace(
                __cache_key__=(f"dynamic-k{bits}",),
                num_experts=counts[index],
            ),
        )
    kernel.tier_count = 4
    kernel.blocks_per_sm = 1
...
def test_four_tier_cache_key_is_partition_agnostic() -> None:
    assert _mixed4_cache_key((128, 64, 48, 16)) == _mixed4_cache_key((144, 48, 40, 24))
    assert _mixed4_cache_key((128, 64, 48, 16)) != _mixed_cache_key(192, 64)
...
def test_mixed_kernel_uses_runtime_expert_bounds() -> None:
    ...
    assert "tier2_num_experts" in call_parameters
    assert "tier3_num_experts" in call_parameters
    assert "local_expert < tier2_num_experts" in emit_source
    assert "local_expert < tier3_num_experts" in emit_source
...
def test_build_multitier_maps_preserves_tier_dispatch() -> None:
    global_to_combined, descriptor = build_multitier_maps(
        ((3, 0), (5, 2), (1,), (4,)), device=torch.device("cpu")
    )
    assert global_to_combined.tolist() == [1, 4, 3, 0, 5, 2]
    assert descriptor.tolist() == [0, 1, 1 << 8, (1 << 8) | 1, 2 << 8, 3 << 8]
    three_map, three_descriptor = build_multitier_maps(
        ((2, 0), (3,), (1,)), device=torch.device("cpu")
    )
    assert three_map.tolist() == [1, 3, 0, 2]
    assert three_descriptor.tolist() == [0, 1, 1 << 8, 2 << 8]

    tiers = tuple(
        SimpleNamespace(
            intermediate_rotations=torch.full((1, 1), index, dtype=torch.float16),
            gate_suh=torch.full((1, 1), index, dtype=torch.float16),
            up_suh=torch.full((1, 1), index, dtype=torch.float16),
            down_svh=torch.full((1, 1), index, dtype=torch.float16),
        )
        for index in range(4)
    )
    rotations = combine_trellis_rotations(*tiers)
    assert rotations.intermediate[:, 0].tolist() == [0.0, 1.0, 2.0, 3.0]


@pytest.mark.skipif(not _sm12x_available(), reason="requires an SM120/SM121 GPU")
def test_mixed_k3_k4_k5_k6_shared_h_matches_serial_and_captures() -> None:
    """A four-tier one-grid launch preserves serial shared-H arithmetic."""

    torch.manual_seed(20260805)
    device = torch.device("cuda", torch.cuda.current_device())
    m, hidden, intermediate, topk = 2, 128, 128, 8
    ...
    topk_weights = torch.softmax(
        torch.tensor(
            [list(range(8, 0, -1)), list(range(1, 9))],
            dtype=torch.float32,
            device=device,
        ),
        dim=-1,
    )
    serial = torch.zeros((m, hidden), dtype=torch.float32, device=device)
    for tier_index, tier in enumerate(tiers):
        mapping = torch.full((8,), -1, dtype=torch.int32, device=device)
        mapping[tier_index * 2 : tier_index * 2 + 2] = torch.arange(
            2, dtype=torch.int32, device=device
        )
        serial.add_(_serial_tier(x, tier, topk_weights, topk_ids, mapping))
    ...
    mixed = run().clone()
    torch.cuda.synchronize(device)
    relative = (mixed - serial).norm() / serial.norm().clamp_min(1.0e-12)
    assert float(relative) < 4.0e-3
    assert torch.equal(run(), mixed)
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = run()
    graph.replay()
    torch.cuda.synchronize(device)
    assert torch.equal(captured, mixed)
