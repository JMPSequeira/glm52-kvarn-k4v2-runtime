...
from .route_pack import fused_sigmoid_topk_pack
...
_FUSED_ROUTE_PACK_WARMED: set[tuple[object, ...]] = set()
    ...
    with torch.cuda.device(device):
        ...
        if correction_bias is not None:
            if routed_scale is None:
                raise ValueError("fused route-pack warmup requires routed_scale")
            fused_key = (
                str(correction_bias.dtype),
                device_index,
                str(launch.route_ids_dtype),
                int(launch.top_k),
                total_experts,
            ...
            if fused_key not in _FUSED_ROUTE_PACK_WARMED:
                # One M_CAPACITY=9 specialization covers every exact decode
                # shape M=1..9; active_m remains a runtime scalar.
                dummy_router_logits = torch.zeros(
                    (9, total_experts), dtype=torch.float32, device=device
                )
                fused_sigmoid_topk_pack(
                    dummy_router_logits,
                    correction_bias,
                    expert_map,
                    ...
                    expert_offsets=buffers.expert_offsets,
                    expert_counts=buffers.expert_counts,
                    block_size=launch.moe_block_size,
                )
                torch.cuda.current_stream(device).synchronize()
                _FUSED_ROUTE_PACK_WARMED.add(fused_key)
                warmed += 1
    return warmed
...
def make_mixed_trellis_buffers(
    ...
    *,
    ...
    shared_scratch = torch.empty(
        max(fc1_scratch_elements, fc2_scratch_elements),
        dtype=torch.float32,
        device=device,
    )
    return MixedTrellisBuffers(
        ...
        topk_weights=torch.empty(
            (launch.size_m, launch.top_k), dtype=torch.float32, device=device
        ),
        topk_ids=torch.empty(
            (launch.size_m, launch.top_k), dtype=torch.int64, device=device
        ),
        ...
        fc1_scratch=shared_scratch[:fc1_scratch_elements],
        fc2_scratch=shared_scratch[:fc2_scratch_elements],
...
def run_mixed_trellis(
    ...
    *,
    ...
    if routes_prepacked:
        packed = buffers.packed_route_indices
...
def run_mixed_trellis_monolithic(
    x: torch.Tensor,
    ...
    *,
    ...
    """Fuse target sigmoid TopK with route packing before mixed Trellis."""
    m = int(x.shape[0])
    if not 1 <= m <= 9 or launch.size_m < m or launch.top_k != 8:
        raise ValueError("monolithic mixed Trellis requires M<=9 and K8")
    fused_sigmoid_topk_pack(
        router_logits,
        correction_bias,
        global_to_combined,
