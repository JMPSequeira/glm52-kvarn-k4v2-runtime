    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if is_kvarn:
            if cache_config.enable_prefix_caching:
                raise ValueError(
                    "KVarN does not support prefix caching. "
            ...
            if cache_dtype in ("kvarn_mla_k5_g64", "kvarn_nvfp4_ds_mla"):
                ...
                if backend not in (
                    ...
                    raise ValueError(
                        f"{cache_dtype} requires --attention-backend B12X_MLA_SPARSE."
                    )
                ...
                if spec_config is not None and getattr(
                    ...
                    if getattr(spec_config, "kv_cache_dtype", None) != cache_dtype:
                        raise ValueError(
                            "MLA KVarN speculative decoding requires explicit "
                            f"kv_cache_dtype={cache_dtype!r}."
                        )
                if cache_config.block_size != 64:
                    raise ValueError(
                        f"{cache_dtype} requires --block-size 64; "
