...
class Platform:
    ...
    def _align_hybrid_block_size(
        ...
        # Compute attention page size for one token. KVarN's packed payload is
        # group-locked and cannot use FullAttentionSpec's raw uint8 formula.
        is_standard_kvarn = (
        ...
        elif cache_config.cache_dtype == "kvarn_nvfp4_ds_mla":
            from vllm.model_executor.layers.quantization.kvarn.config import (
                KVarNMLAConfig,
            )

            kvarn_mla_config = KVarNMLAConfig.from_cache_dtype(cache_config.cache_dtype)
            attn_page_size_1_token = (
                kvarn_mla_config.packed_page_bytes // kvarn_mla_config.group
            )
