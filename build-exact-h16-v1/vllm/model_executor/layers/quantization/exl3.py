...
class Exl3Config(QuantizationConfig):
    ...
    def _configure_rank_sliced(self, metadata: dict[str, Any]) -> None:
        ...
        raw_bits = metadata["trellis_bits_by_layer"]
        if not isinstance(raw_bits, dict):
            raise ValueError("rank-sliced EXL3 trellis_bits_by_layer must be an object")
        invalid_keys = [
            repr(key)
            for key in raw_bits
        ...
        if invalid_keys:
            raise ValueError(
                "rank-sliced EXL3 trellis_bits_by_layer keys must be decimal "
                f"layer strings, got {invalid_keys[:8]}"
            )
        ...
        for layer_key, bits in raw_bits.items():
            if (
                ...
                raise ValueError(
                    ...
                    f"MCG3/4/5/6/8 values, got layer {layer_key}={bits!r}"
                )
    ...
    def trellis_bits_for_layer(self, prefix: str) -> int:
        if self.trellis_bits_by_layer is None:
            raise ValueError("EXL3 checkpoint has no per-layer Trellis precision map")
    ...
    def _validate_storage_metadata(self) -> None:
        ...
        for prefix, entry in self.tensor_storage.items():
            if not isinstance(entry, dict):
                bad.append(f"{prefix!r}: tensor record must be an object")
                continue
            if entry.get("quant_format") != "exl3":
                continue
            exl3_count += 1
            if not isinstance(prefix, str) or not prefix:
                bad.append(f"{prefix!r}: prefix must be a non-empty string")
                continue
            stored = entry.get("stored_tensors")
            if not isinstance(stored, dict):
                bad.append(f"{prefix}: stored_tensors must be an object")
                continue
            if not all(isinstance(name, str) for name in stored):
                bad.append(f"{prefix}: stored tensor names must be strings")
                continue
...
class Exl3LinearMethod(LinearMethodBase):
    ...
    def _pad_lm_head_tensors_for_tensor_parallel(layer: torch.nn.Module) -> None:
        ...
        for shard_id, trellis, svh in loaded:
            ...
            # zero svh therefore produces exact zero without touching or mixing
            # any stored output row, irrespective of its valid zero trellis code.
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if rank_sliced:
            layer.exl3_trellis_bits = self.quant_config.trellis_bits_for_layer(
                layer.layer_name
            )
    ...
    def _validate_moe_shapes(layer: RoutedExperts) -> None:
        expected_bits = int(layer.exl3_trellis_bits)
        for expert_id in range(layer.local_num_experts):
            for group, shard_ids in (("w13", ("w1", "w3")), ("w2", ("w2",))):
                for shard_id in shard_ids:
                    ...
                    if (
                        ...
                        or trellis.ndim != 3
                        or trellis.shape[2] != 16 * expected_bits
    ...
    def _rank_sliced_runtime(
        ...
        exact_decode_max_m = int(os.environ.get("VLLM_EXL3_TRELLIS_EXACT_MAX_M", "0"))
        if exact_decode_max_m < 0:
            raise ValueError(
                "VLLM_EXL3_TRELLIS_EXACT_MAX_M must be nonnegative, "
                f"got {exact_decode_max_m}"
            )
        exact_decode_block_m = _positive_env_int("VLLM_EXL3_TRELLIS_EXACT_BLOCK_M", 8)
        ...
        if exact_decode_max_m and not (
            min_trellis_m <= exact_decode_max_m <= max_trellis_m
        ):
            raise ValueError(
                "VLLM_EXL3_TRELLIS_EXACT_MAX_M must lie inside the decode "
                f"window [{min_trellis_m}, {max_trellis_m}], "
                f"got {exact_decode_max_m}"
            )
        if exact_decode_max_m and exact_decode_block_m not in (8, 16, 32, 48, 64):
            raise ValueError(
                "VLLM_EXL3_TRELLIS_EXACT_BLOCK_M must be one of "
                "{8, 16, 32, 48, 64}, "
                f"got {exact_decode_block_m}"
            )
        ...
        def _build_plans() -> dict[str, Any]:
            ...
            if exact_decode_max_m:
                plans["exact_decode_plan"] = _make_plan(
                    exact_decode_max_m, exact_decode_block_m
                )
            else:
                plans["exact_decode_plan"] = None
        ...
        if prefill_scratch is None:
            ...
            _validate_shared_scratch(
                plans["exact_decode_plan"], prefill_scratch, "exact-decode"
            )
            _validate_shared_scratch(
        ...
        runtime = {
            ...
            "exact_decode_max_m": exact_decode_max_m,
            "exact_decode_block_m": exact_decode_block_m,
            "max_batched_tokens": max_batched_tokens,
        ...
        logger.info_once(
            ...
            "exact-decode %s, mid-prefill %s, small-prefill %s, prefill %s "
            "capacity=%d chunk=%d topk=%d",
            min_trellis_m,
            max_trellis_m,
            block_m,
            (
                f"m<={exact_decode_max_m} block_m={exact_decode_block_m}"
                if exact_decode_max_m
                else "disabled"
            ),
            (
    ...
    def _apply_rank_sliced(
        ...
        m = int(x.shape[0])
        decode_plan = plans["trellis_plan"]
        if (
            plans["exact_decode_plan"] is not None
            and runtime["min_trellis_m"] <= m <= runtime["exact_decode_max_m"]
        ):
            decode_plan = plans["exact_decode_plan"]
...
def get_exl3_mcg8_mla_packed_weights(
    ...
    # Every representable local shard starts on an even 16-head boundary.
