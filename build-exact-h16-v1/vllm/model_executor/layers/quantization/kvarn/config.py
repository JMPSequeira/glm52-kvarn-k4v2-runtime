...
_MLA_PRECISION_TAIL_TOKENS_ENV = "KVARN_MLA_PRECISION_TAIL_TOKENS"
_BF16_BYTES = 2
_INT32_BYTES = 4
_MAX_TENSOR_BYTES = (1 << 63) - 1
_MLA_K5_PACKED_PAGE_BYTES = 30_848
_MLA_NVFP4_PACKED_PAGE_BYTES = 24_576
KVarNMLAPackedFormat = Literal["k5_g64", "nvfp4_ds_g64"]
...
def kvarn_mla_workspace_envelope(
    *,
    ...
    """Return the fixed graph-static staging envelope for one local worker."""
    dimensions = {
    ...
    if unsupported:
        raise ValueError(f"Unsupported KVarN MLA workspace dimensions: {unsupported}")
    invalid = {
        name: value
        for name, value in dimensions.items()
        if value <= 0 and name not in {"boundary_blocks", "rollback_blocks"}
    }
    ...
    allocations = {
        ...
        "remap_bytes": remap_elements * _INT32_BYTES,
        "rotation_bytes": max_batched_tokens * latent_dim * _BF16_BYTES,
        "physical_slot_bytes": page_rows * _INT32_BYTES,
        "total_bytes": total_bytes,
    }
    overflow = {
        name: value for name, value in allocations.items() if value > _MAX_TENSOR_BYTES
    }
    if overflow:
        raise OverflowError(f"KVarN MLA workspace envelope overflows int64: {overflow}")
...
def is_deepseek_mtp_draft_model_config(model_config: object) -> bool:
    """Identify an explicit speculative deepseek_mtp ModelConfig."""
    hf_config = getattr(model_config, "hf_config", None)
    return (
        getattr(model_config, "spec_target_max_model_len", None) is not None
        and getattr(hf_config, "model_type", None) == "deepseek_mtp"
    )


def is_deepseek_mtp_draft(vllm_config: "VllmConfig") -> bool:
    """Identify the actual speculative MTP model, never the target backend."""
    return is_deepseek_mtp_draft_model_config(vllm_config.model_config)
...
class KVarNMLAConfig:
    """Format-specific packed G64 MLA cache plus a shared exact side pool.

    Live boundary, precision-tail, current, and rollback blocks always use FP8
    E4M3 latent and BF16 RoPE side storage. Retired blocks use ``packed_format``.
    """
    ...
    bits: int = 5
    group: int = 64
    packed_format: KVarNMLAPackedFormat = "k5_g64"
    sinkhorn_iters: int = 8
    precision_tail_tokens: int = 0
    """Recent global logical tokens retained in the exact latent/RoPE pool."""
    boundary_tokens: ClassVar[int] = 128

    @property
    def is_k5(self) -> bool:
        return self.packed_format == "k5_g64"

    @property
    def is_nvfp4_ds(self) -> bool:
        return self.packed_format == "nvfp4_ds_g64"

    @property
    def packed_page_bytes(self) -> int:
        if self.is_k5:
            return _MLA_K5_PACKED_PAGE_BYTES
        if self.is_nvfp4_ds:
            return _MLA_NVFP4_PACKED_PAGE_BYTES
        raise ValueError(f"Unsupported KVarN MLA packed format: {self.packed_format!r}")

    @property
    def nvfp4_row_bytes(self) -> int:
        return 368

    @property
    def nvfp4_channel_footer_offset(self) -> int:
        return self.group * self.nvfp4_row_bytes
    ...
    @property
    def tile_bytes(self) -> int:
        """Packed bytes charged for one G64 cache page."""
        return self.packed_page_bytes
    ...
    @property
    def resident_blocks_per_seq(self) -> int:
        """Worst-case boundary/tail/current union for one unsharded request."""
        # A logical tail can straddle one more group than ceil(tokens / group).
        # Its newest group is also the current group, so count their union once.
        tail_blocks = (
            math.ceil((self.precision_tail_tokens + self.group - 1) / self.group)
            if self.precision_tail_tokens
            else 1
        )
        return math.ceil(self.boundary_tokens / self.group) + tail_blocks
    ...
    def pool_slots(
        ...
        async_epoch_blocks = max_num_seqs if max_rollback_tokens else 0
        return max(
            resident
            + prefill_blocks
            + boundary_overlap
    ...
    def mtp_full_context_pool_slots(
        self,
        max_num_seqs: int,
        max_num_batched_tokens: int,
        max_model_len: int,
        ...
        """Exact-pool budget for a deepseek_mtp cache shard only."""
        dcp_world_size = max(dcp_world_size, 1)
        dcp_interleave = max(dcp_interleave, 1)
        max_local_tokens = (
            math.ceil(max_model_len / dcp_world_size) + dcp_interleave - 1
        )
        full_blocks = math.ceil(max_local_tokens / self.group)
        resident = full_blocks * max_num_seqs
        prefill_blocks = math.ceil(
            (math.ceil(max_num_batched_tokens / dcp_world_size) + dcp_interleave - 1)
            / self.group
        )
        rollback_blocks = (
            math.ceil(
                (
                    math.ceil(max_num_seqs * max_rollback_tokens / dcp_world_size)
                    + dcp_interleave
                    - 1
                )
                / self.group
        ...
        # A stale generation can coexist with all active exact contexts until
        # its asynchronous rollback epoch resolves; reserve that fail-closed case.
        async_epoch_blocks = resident if max_rollback_tokens else 0
        return max(
            resident + prefill_blocks + rollback_blocks + async_epoch_blocks + 8,
            8,
        )
    ...
    def workspace_envelope(
        ...
        """Size graph-static staging allocated once by a local worker."""
        geometry = (
            self.group,
            self.latent_dim,
            self.rope_dim,
            self.bits,
            self.packed_format,
            self.packed_page_bytes,
        )
        supported = {
            (64, 512, 64, 5, "k5_g64", _MLA_K5_PACKED_PAGE_BYTES),
            (64, 512, 64, 4, "nvfp4_ds_g64", _MLA_NVFP4_PACKED_PAGE_BYTES),
        }
        if geometry not in supported:
            raise ValueError(f"Unsupported KVarN MLA workspace geometry {geometry}")
        scheduler_config = vllm_config.scheduler_config
        max_num_seqs = int(scheduler_config.max_num_seqs)
    ...
    def from_cache_dtype(cls, cache_dtype: str) -> "KVarNMLAConfig":
        formats: dict[str, tuple[KVarNMLAPackedFormat, int]] = {
            "kvarn_mla_k5_g64": ("k5_g64", 5),
            "kvarn_nvfp4_ds_mla": ("nvfp4_ds_g64", 4),
        }
        try:
            packed_format, bits = formats[cache_dtype]
        except KeyError as exc:
            valid = ", ".join(formats)
            raise ValueError(
                f"Unknown MLA KVarN cache dtype {cache_dtype!r}; valid: {valid}"
            ) from exc
        precision_tail_tokens = int(os.environ.get(_MLA_PRECISION_TAIL_TOKENS_ENV, "0"))
        if precision_tail_tokens < 0:
            raise ValueError(
                f"{_MLA_PRECISION_TAIL_TOKENS_ENV} must be non-negative, "
                f"got {precision_tail_tokens}"
            )
        return cls(
            bits=bits,
