...
class SharedExperts(torch.nn.Module):
    ...
    def maybe_sync_shared_experts_stream(
        ...
        if experts_order == SharedExpertsOrder.MULTI_STREAM_OVERLAPPED:
            ...
            # Keep the input alive on the auxiliary stream and order that stream
            # after the producer. Submit before gate/router work so those kernels
            # can overlap; the main stream joins immediately before routed MoE.
    ...
    def _join_aux_stream(self) -> None:
        ...
        # The output allocation belongs to the auxiliary stream but is consumed
        # on the main stream after the join.
