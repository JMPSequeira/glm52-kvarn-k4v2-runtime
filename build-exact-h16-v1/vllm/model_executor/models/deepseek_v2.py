...
def _indexer_cache_dcp_shard_count(prefix: str, vllm_config: VllmConfig) -> int:
    """Select the target indexer's physical DCP shard geometry."""
    ...
    if (
        ...
        or int(layer_id) >= int(num_hidden_layers)
    ):
        # MTP/draft caches retain the configured physical namespace.
        return configured_cp_size

    requested_shards = getattr(
        parallel_config, "dcp_indexer_shards", configured_cp_size
    )
    if requested_shards != configured_cp_size and not getattr(
        vllm_config, "use_v2_model_runner", False
    ):
        raise NotImplementedError(
            "Partially replicated sparse-indexer KV requires the V2 model runner"
        )
    if requested_shards != configured_cp_size and not use_b12x_sparse_indexer():
        raise RuntimeError(
