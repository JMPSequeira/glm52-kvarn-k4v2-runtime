...
class KVCacheSpec:
    ...
    def is_uniform_with_collection(
        ...
        # Cache groups carry one physical CP geometry. Keep partial, replicated,
        # and fully sharded layers in distinct groups.
        self_layout = (
...
class AttentionSpec(KVCacheSpec):
    ...
    def max_num_blocks_per_req(self, vllm_config: VllmConfig, max_len: int) -> int:
        # Attention KV is token-interleaved across the cache group's physical
        # CP shards. Partially replicated groups intentionally use fewer than
        # the configured DCP ranks.
        parallel_config = vllm_config.parallel_config
        cp_shards = get_kv_cache_cp_shard_count(
            self,
            parallel_config.decode_context_parallel_size,
            parallel_config.prefill_context_parallel_size,
        )
        return cdiv(max_len, self.block_size * cp_shards)
...
class FullAttentionSpec(AttentionSpec):
    ...
    """Optional number of physical KV shards inside the configured DCP group.

    ``None`` uses every configured DCP/PCP rank. A proper divisor creates
    replicated shard groups while preserving global logical block IDs.
    """
...
def has_nondefault_kv_cp_layout(
    ...
    return get_kv_cache_cp_shard_count(spec, dcp_world_size, pcp_world_size) != (
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    def real_page_size_bytes(self) -> int:
        if self.cache_dtype_str in (
            ...
            if self.block_size != config.group:
                raise ValueError(
                    f"MLA KVarN requires block_size={config.group}, "
                    f"got {self.block_size}"
                )
            return config.packed_page_bytes
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        dcp_kv_shard_count_set = {spec.dcp_kv_shard_count for spec in specs}
        assert (
