...
def _b12x_dcp_lse_reduce_enabled() -> bool:
    return bool(int(os.getenv("VLLM_B12X_DCP_LSE_REDUCE", "1")))
...
def dcp_b12x_all_gather_heads_into(
    local_input: torch.Tensor,
    cp_group: GroupCoordinator,
    out: torch.Tensor,
    *,
    max_batch_size: int | None = None,
    pool_head_dim: int | None = None,
) -> torch.Tensor | None:
    """Gather rank-local query heads into caller-owned ``out`` (B12X only).

    The B12X PCIe channel writes the gathered heads directly into ``out``
    (shape ``[batch, total_heads, head_dim]``, contiguous, on the pool device),
    eliminating the separate destination copy of the allocating variant.
    Returns ``out`` on success or ``None`` when the channel, geometry, or
    buffers do not qualify -- callers must then fall back to their incumbent
    gather path (fail closed). The NCCL group backend has no ``out=`` form,
    so unlike :func:`dcp_b12x_all_gather_heads` there is no silent fallback
    here by design.
    """
    ...
    if (
        ...
        or local_input.ndim != 3
        or local_input.shape[0] != out.shape[0]
        or local_input.shape[2] != out.shape[2]
        or local_input.dtype != out.dtype
        or local_input.device != out.device
    ):
        return None
    world_size = cp_group.world_size
    if (
        world_size not in (2, 4, 8)
        or local_input.shape[1] * world_size != out.shape[1]
        or local_input.dtype not in (torch.float16, torch.bfloat16)
        or local_input.shape[2] % 8 != 0
        or local_input.shape[1] <= 0
    ):
        return None
    ...
    if pool_head_dim is None:
        pool_head_dim = int(out.shape[2])
    # ``pool_head_dim`` lets callers share the pooled channel their incumbent
    # allocating gather already created (its ``head_dim`` sizes only the
    # lse-reduce output; all_gather_heads exchanges ``query_head_dim`` rows).
    pool = _get_b12x_dcp_a2a_pool(
        cp_group,
        device=local_input.device,
        total_heads=int(out.shape[1]),
        head_dim=int(pool_head_dim),
        query_head_dim=int(local_input.shape[2]),
        max_batch_size=max_batch_size,
    )
    if pool is None:
        return None
    try:
        return pool.all_gather_heads(local_input, out)
    except Exception:
        # Geometry or capacity mismatch against the pooled channel: the
        # caller's incumbent path is the correct answer, not an error.
        return None
...
def warmup_b12x_dcp_a2a(
    ...
    *,
    ...
    if not _b12x_dcp_lse_reduce_enabled():
        return
...
def dcp_a2a_lse_reduce(
    ...
    if (
        ...
        and _b12x_dcp_lse_reduce_enabled()
    ):
