...
def _bf16_mla_sparse_kernel(
    ...
    # A DCP rank can own no selected KV rows for a short sequence. Return the
    # neutral partial-attention state instead of dividing by zero.
    nonempty = e_sum > 0
    acc /= tl.where(nonempty, e_sum, 1.0)[:, None]
    acc = tl.where(nonempty[:, None], acc, 0.0)

    max_logits = tl.where(nonempty, e_max * LOGE2, -float("inf"))
    # calculate lse
    lse = tl.where(
        nonempty,
        max_logits + tl.log2(e_sum) * LOGE2,
        -float("inf"),
    )
...
def triton_bf16_mla_sparse_interface(
    ...
    """
    out : [num_tokens, num_heads_q, d_v]; when passed in, it is written
        directly and may alias the NoPE prefix of ``q``.
