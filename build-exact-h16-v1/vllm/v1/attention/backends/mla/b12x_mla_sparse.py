...
from vllm.v1.attention.ops.kvarn_mla import direct_packed_kvarn_mla_decode
...
def _kvarn_mla_workspace_envelope(
    *,
    ...
    # The shared BF16 buffer serves diagnostic materialization and transient
    # state. Production extend uses a separately admitted request-major FP8
    # workspace because its graph bound depends on max local sequence length,
    # not the allocated page count.
...
def _direct_packed_kvarn_effective_a2a(num_tokens: int) -> bool:
    token_cap = envs.VLLM_DCP_A2A_MAX_TOKENS
    return (
        token_cap <= 0
        or token_cap >= num_tokens
        or envs.VLLM_DCP_A2A_LARGE_BACKEND == "a2a"
    )


def _use_direct_packed_kvarn_mla_decode(
    *, use_decode_kernel: bool, direct_packed_enabled: bool, num_actual_toks: int
) -> bool:
    return (
        use_decode_kernel
        and direct_packed_enabled
        and num_actual_toks in (1, 4, 16)
        and _direct_packed_kvarn_effective_a2a(num_actual_toks)
    )
...
def _enable_direct_packed_kvarn_mla_instance(
    *,
    requested: bool,
    has_selected_buffer: bool,
    dcp_comm_backend: str | None,
    packed_format: str = "k5_g64",
) -> bool:
    return (
        requested
        and packed_format == "k5_g64"
        and has_selected_buffer
        and dcp_comm_backend in {"a2a", "ag_rs"}
        and all(_direct_packed_kvarn_effective_a2a(rows) for rows in (1, 4, 16))
    )


def _fuse_native_kvarn_hadamard(
    *, native_available: bool, direct_dispatch: bool, model_role: str
) -> bool:
    """Fuse transforms only for the fail-closed native target dispatch."""
    return native_available and direct_dispatch and model_role == "target"


def _native_packed_k5_exact_pool_only(
    *,
    ...
    """Select exact-only decode from a graph-static scheduler proof."""
    if full_context_exact:
        return True
    if graph_exact is not None:
        return graph_exact
    return precision_tail_tokens > 0 and max_seq_len <= precision_tail_tokens


def _validate_kvarn_mla_exact_graph(
    *, graph_exact: bool | None, max_seq_len: int, precision_tail_tokens: int
) -> None:
    """Reject an exact graph whose scheduler length proof is inconsistent."""
    if graph_exact is True and (
        precision_tail_tokens <= 0 or max_seq_len > precision_tail_tokens
    ):
        raise RuntimeError(
            "KVarN MLA exact graph exceeds its precision-tail coverage: "
            f"max_seq_len={max_seq_len}, precision_tail_tokens="
            f"{precision_tail_tokens}"
        )


def _use_target_exact_selected_decode(
    *,
    enabled: bool,
    model_role: str,
    complete_exact_pool: bool,
    native_direct_candidate: bool,
) -> bool:
    """Prefer selected E4M3 staging only for fully exact target decode."""
    return (
        enabled
        and model_role == "target"
        and complete_exact_pool
        and native_direct_candidate
    )


def _dcp_local_candidate_envelope(
    *,
    max_seq_len: int,
    topk_tokens: int,
    dcp_size: int,
    ...
    if max_seq_len <= 0:
        return 0
    base = max_seq_len // interleave // dcp_size * interleave
    remainder = max_seq_len - base * dcp_size
    local_seq_len = base + min(max(remainder - dcp_rank * interleave, 0), interleave)
    return min(topk_tokens, local_seq_len)
...
class B12xMLASparseBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str in ("kvarn_mla_k5_g64", "kvarn_nvfp4_ds_mla"):
            ...
            return (num_blocks, 1, config.packed_page_bytes)
...
class B12xMLASparseMetadata(AttentionMetadata):
    ...
    kvarn_direct_candidate_envelope: int = 0
    kvarn_mla_exact: bool | None = None
...
class B12xMLASparseMetadataBuilder(AttentionMetadataBuilder[B12xMLASparseMetadata]):
    ...
    def __init__(
        ...
        self._kvarn_ownership_layer_names = list(layer_names)
    ...
    def prepare_kvarn_mla_ownership(
        self,
        block_fills: dict[int, int | None],
        excluded_layer_names: set[str] | None = None,
    ) -> None:
        if self.kvarn_config is None:
            return
        if excluded_layer_names is not None:
            self._kvarn_ownership_layer_names = [
                layer_name
                for layer_name in self.layer_names
                if layer_name not in excluded_layer_names
            ]
        if not self._kvarn_ownership_layer_names:
            return
        KVarNMLAStateManager.prepare_step(
            tuple(self._kvarn_ownership_layer_names),
            self._kvarn_ownership_layer_names,
            block_fills,
            self.kvarn_config,
        )
    ...
    def build(
        ...
        if self.kvarn_config is not None and cm.for_cudagraph_capture:
            self.prepare_kvarn_mla_ownership({0: None} if cm.kvarn_mla_exact else {})
        ...
        if self.kvarn_config is not None and not cm.for_cudagraph_capture:
            exact_max_seq_len = (
                cm.batch_topology.max_seq_len_upper_bound
                if cm.batch_topology is not None
                else cm.max_seq_len
            )
            _validate_kvarn_mla_exact_graph(
                graph_exact=cm.kvarn_mla_exact,
                max_seq_len=exact_max_seq_len,
                precision_tail_tokens=self.kvarn_config.precision_tail_tokens,
            )
        return B12xMLASparseMetadata(
            ...
            kvarn_direct_candidate_envelope=(
                _dcp_local_candidate_envelope(
                    max_seq_len=cm.max_seq_len,
                    topk_tokens=self.topk_tokens,
            ...
            kvarn_mla_exact=cm.kvarn_mla_exact,
        )
...
class B12xMLASparseImpl(MLAAttentionImpl[B12xMLASparseMetadata]):
    ...
    _kvarn_shared_live_stage: ClassVar[dict[tuple, torch.Tensor]] = {}
    _kvarn_shared_selected_stage: ClassVar[dict[tuple, torch.Tensor]] = {}
    _kvarn_shared_selected_physical_slots: ClassVar[dict[tuple, torch.Tensor]] = {}
    ...
    _kvarn_shared_block_to_logical: ClassVar[dict[tuple, torch.Tensor]] = {}
    ...
    def __init__(
        ...
        **mla_args,
        ...
        self._is_kvarn_mla = self.kv_cache_dtype in (
            "kvarn_mla_k5_g64",
            "kvarn_nvfp4_ds_mla",
        )
        self._is_kvarn_nvfp4_mla = self.kv_cache_dtype == "kvarn_nvfp4_ds_mla"
        self._is_deepseek_mtp_draft = False
        self._kvarn_config = None
        if self._is_kvarn_mla:
            from vllm.model_executor.layers.quantization.kvarn.config import (
                KVarNMLAConfig,
                is_deepseek_mtp_draft,
            )
        ...
        if self._is_kvarn_mla:
            self._is_deepseek_mtp_draft = is_deepseek_mtp_draft(vllm_config)
        parallel_config = vllm_config.parallel_config
        ...
        native_cute_mode = os.getenv("VLLM_KVARN_MLA_NATIVE_CUTE", "0").strip()
        if native_cute_mode not in {"0", "1"}:
            raise ValueError("VLLM_KVARN_MLA_NATIVE_CUTE must be exactly 0 or 1")
        self._kvarn_native_cute_decode = native_cute_mode == "1"
        exact_use_selected_mode = os.getenv(
            "VLLM_KVARN_MLA_EXACT_USE_SELECTED", "0"
        ).strip()
        if exact_use_selected_mode not in {"0", "1"}:
            raise ValueError("VLLM_KVARN_MLA_EXACT_USE_SELECTED must be exactly 0 or 1")
        self._kvarn_exact_use_selected = exact_use_selected_mode == "1"
        bf16_decode_diagnostic = os.getenv(
            "VLLM_KVARN_MLA_BF16_DECODE_DIAGNOSTIC", "0"
        ).strip()
        if bf16_decode_diagnostic not in {"0", "1"}:
            raise ValueError(
                "VLLM_KVARN_MLA_BF16_DECODE_DIAGNOSTIC must be exactly 0 or 1"
            )
        self._kvarn_bf16_decode_diagnostic = bf16_decode_diagnostic == "1"
        if self._is_kvarn_mla and self._kvarn_bf16_decode_diagnostic:
            logger.warning_once(
                "Using opt-in BF16 KVarN MLA decode diagnostics instead of "
                "compact FP8 staging and native B12X decode"
            )
        has_selected_buffer = _direct_packed_selected_buffer_valid(
            self.topk_indices_buffer, self.device
        )
        self._kvarn_direct_index_source = (
            "local-indexer" if indexer is not None else "shared-index"
        )
        hf_config = (
        ...
        self._kvarn_direct_model_role = (
            "draft"
            if getattr(hf_config, "model_type", None) == "deepseek_mtp"
            else "target"
        )
        self._kvarn_native_packed_decode: Any | None = None
        self._kvarn_validated_direct_rows: set[int] = set()
        self._kvarn_validated_direct_scratch_storages: set[int] = set()
        self._kvarn_direct_packed_decode = _enable_direct_packed_kvarn_mla_instance(
            requested=direct_packed_requested,
            has_selected_buffer=has_selected_buffer,
            dcp_comm_backend=dcp_comm_backend,
            packed_format=(
                self._kvarn_config.packed_format
                if self._kvarn_config is not None
                else ""
            ),
        )
        if (
            direct_packed_requested
            and self._is_kvarn_mla
            and not self._is_kvarn_nvfp4_mla
            and not self._kvarn_direct_packed_decode
        ):
            raise RuntimeError(
                "Direct packed KVarN MLA was requested but its graph-static "
                "selected buffer or DCP A2A contract is unavailable: "
                f"selected_buffer_valid={has_selected_buffer}, "
                f"dcp_comm_backend={dcp_comm_backend!r}"
            )
        if self._kvarn_direct_packed_decode:
            ...
            contract = {
                ...
                "index_source": self._kvarn_direct_index_source,
                "model_role": self._kvarn_direct_model_role,
            }
            if (
                not self._is_kvarn_mla
                or geometry != (64, 512, 64, 5, 30_848)
                or self.tp_world_size != 4
                or self.dcp_world_size != 4
                or dcp_comm_backend not in {"a2a", "ag_rs"}
                or not all(
                    _direct_packed_kvarn_effective_a2a(rows) for rows in (1, 4, 16)
                )
                ...
                raise ValueError(
                    ...
                    "K5/G64 SM120 TP4/DCP4/A2A/H16->H64/topk2048 contract; "
                    f"got {contract}"
                )

        if self._kvarn_direct_packed_decode and self._kvarn_native_cute_decode:
            try:
                from sparkinfer.attention.kvarn_mla import native_packed_k5_decode
            except ImportError as exc:
                raise RuntimeError(
                    "Native packed K5 decode was requested but SparkInfer does "
                    "not provide native_packed_k5_decode"
                ) from exc
        ...
        if self._is_kvarn_mla:
            ...
            max_local_tokens = (
                _cdiv(
                    ...
                    max(self.dcp_world_size, 1),
                )
                + self.cp_kv_cache_interleave_size
            )
            self._kvarn_live_slot_rows = (
                _cdiv(max_local_tokens * max_num_seqs, kvarn_config.group)
                * kvarn_config.group
            )
            max_rollback_tokens = (
            ...
            if self._is_deepseek_mtp_draft:
                self._kvarn_pool_size = kvarn_config.mtp_full_context_pool_slots(
                    max_num_seqs,
                    max_batched,
                    int(vllm_config.model_config.max_model_len),
                    max(self.dcp_world_size, 1),
            ...
            self._kvarn_live_stage: torch.Tensor | None = None
            self._kvarn_selected_stage: torch.Tensor | None = None
            self._kvarn_selected_physical_slots: torch.Tensor | None = None
            self._kvarn_diag_selected_indices: torch.Tensor | None = None
            self._kvarn_diag_valid_counts: torch.Tensor | None = None
    ...
    def initialize_kvarn_workspaces(
        ...
        for impl in matching:
            ...
            selected_physical_bytes = selected_rows * _INT32_BYTES
            live_stage_bytes = impl._kvarn_live_slot_rows * 656
            live_physical_bytes = impl._kvarn_live_slot_rows * _INT32_BYTES
            block_lookup_bytes = num_kv_pages * _INT32_BYTES
            direct_stage_bytes = envelope.physical_slot_rows * 656
            workspace_bytes = (
                envelope.total_bytes
                - envelope.physical_slot_rows * _INT32_BYTES
                + live_stage_bytes
                + live_physical_bytes
                + block_lookup_bytes
            ...
            if impl._kvarn_direct_packed_decode:
                workspace_bytes += direct_stage_bytes
            key = (
                device,
                num_kv_pages,
                envelope,
                ...
                impl._kvarn_live_slot_rows,
                config.group,
                config.latent_dim,
                config.rope_dim,
                config.packed_format,
            ...
            if key not in cls._kvarn_shared_dense:
                ...
                physical_slots = torch.empty(
                    impl._kvarn_live_slot_rows,
                    dtype=torch.int32,
                    device=device,
                )
                block_to_logical = torch.empty(
                    num_kv_pages,
                    dtype=torch.int32,
                    device=device,
                )
                ...
                cls._kvarn_shared_live_stage[key] = live_stage
                cls._kvarn_shared_selected_stage[key] = selected_stage
                cls._kvarn_shared_selected_physical_slots[key] = selected_physical_slots
            ...
            impl._kvarn_block_to_logical = cls._kvarn_shared_block_to_logical[key]
    ...
    def reset_kv_cache_binding_state(cls) -> None:
        ...
        for impl in tuple(cls._kvarn_instances):
            ...
            impl._kvarn_live_stage = None
            impl._kvarn_selected_stage = None
            impl._kvarn_selected_physical_slots = None
            impl._kvarn_diag_selected_indices = None
            impl._kvarn_diag_valid_counts = None
        ...
        cls._kvarn_shared_live_stage.clear()
        ...
        cls._kvarn_shared_block_to_logical.clear()
    ...
    def _ensure_kvarn_mla_cache(self, kv_cache: torch.Tensor) -> None:
        if not self._is_kvarn_mla:
            return
        if self._kvarn_cache_ref is kv_cache and self._kvarn_block_to_slot is not None:
            return
        ...
        if (
            self._kvarn_live_stage is None
            or self._kvarn_physical_slots is None
            or self._kvarn_block_to_logical is None
        ):
            raise RuntimeError(
                "KVarN MLA graph workspace was not initialized before cache use"
            )
        if self._kvarn_physical_slots.numel() != self._kvarn_live_slot_rows:
            raise RuntimeError("KVarN MLA live-slot workspace geometry changed")
        planned_pages = self._kvarn_block_to_logical.numel()
    ...
    def _flush_kvarn_mla_blocks(
        ...
        if self._kvarn_cache_ref is None or block_ids.numel() == 0:
            return
        assert self._kvarn_config is not None
        if self._kvarn_config.is_nvfp4_ds:
            from vllm.v1.attention.ops.kvarn_mla import (
                pack_kvarn_nvfp4_mla_blocks as pack_blocks,
            )
        else:
            from vllm.v1.attention.ops.kvarn_mla import (
                pack_kvarn_mla_blocks as pack_blocks,
            )
    ...
    def _stage_kvarn_mla_fp8_cache(
        ...
        if (
            ...
            raise RuntimeError("KVarN MLA graph workspace is not initialized")
        from vllm.v1.attention.ops.kvarn_mla import (
            stage_live_kvarn_mla_fp8_batch,
        )
        ...
        max_blocks_per_req = min(
            attn_metadata.block_table.shape[1],
            _cdiv(max_local_seq_len, self._kvarn_config.group),
        )
        num_reqs = attn_metadata.num_reqs
        dense_rows = num_reqs * max_blocks_per_req * self._kvarn_config.group
        required_bytes = dense_rows * 656
        byte_workspace = self._kvarn_live_stage.view(-1)
        if byte_workspace.numel() < required_bytes:
            raise ValueError(
                "KVarN MLA live FP8 workspace has "
                f"{byte_workspace.numel()} bytes, requires {required_bytes}"
            )
        if self._kvarn_physical_slots.numel() < dense_rows:
            raise ValueError(
                "KVarN MLA live-slot workspace has "
                f"{self._kvarn_physical_slots.numel()} rows, requires {dense_rows}"
            )
        records = byte_workspace[:required_bytes].view(dense_rows, 656)
        stage_live_kvarn_mla_fp8_batch(
            attn_metadata.block_table[:num_reqs, :max_blocks_per_req],
            attn_metadata.cache_seq_lens_per_req[:num_reqs],
            selected_indices,
            kv_cache,
            self._kvarn_block_to_slot,
            self._kvarn_latent_pool,
    ...
    def forward_mqa(
        ...
        complete_exact_pool = bool(
            ...
            and _native_packed_k5_exact_pool_only(
                full_context_exact=self._is_deepseek_mtp_draft,
                max_seq_len=attn_metadata.max_seq_len,
                precision_tail_tokens=self._kvarn_config.precision_tail_tokens,
                graph_exact=attn_metadata.kvarn_mla_exact,
            )
        )
        native_direct_candidate = (
            complete_exact_pool
            and self._kvarn_direct_model_role == "target"
            and self._is_kvarn_mla
            and _use_direct_packed_kvarn_mla_decode(
                use_decode_kernel=use_decode_kernel,
                direct_packed_enabled=self._kvarn_direct_packed_decode,
                num_actual_toks=int(query_rows),
            )
        )
        target_exact_selected_decode = _use_target_exact_selected_decode(
            enabled=self._kvarn_exact_use_selected,
            model_role=self._kvarn_direct_model_role,
            complete_exact_pool=complete_exact_pool,
            native_direct_candidate=native_direct_candidate,
        )
        direct_dispatch = native_direct_candidate and not target_exact_selected_decode
        fuse_kvarn_hadamard = _fuse_native_kvarn_hadamard(
            native_available=self._kvarn_native_packed_decode is not None,
        ...
        if isinstance(q, tuple):
            ...
            if self._is_kvarn_mla and not fuse_kvarn_hadamard:
            ...
            if self._is_kvarn_mla and not fuse_kvarn_hadamard:
                q_all[..., self.kv_lora_rank :].copy_(q[..., self.kv_lora_rank :])
                q_all[..., : self.kv_lora_rank].copy_(
                    kvarn_hadamard(
                        q[..., : self.kv_lora_rank],
                        self._kvarn_h,
        ...
        if self._is_kvarn_mla:
            ...
            if direct_dispatch:
                ...
                binding = self._decode_plan.bind(
                    scratch=scratch_storage,
                    q=q_all[:num_actual_toks, : self._input_num_heads],
                    selected_indices=selected_indices,
                    cache_seqlens_int32=cache_seqlens,
                    nsa_cache_seqlens_int32=nsa_cache_seqlens,
                )
                ...
                direct_storage = direct_scratch.tmp_output.untyped_storage().data_ptr()
                if direct_storage not in self._kvarn_validated_direct_scratch_storages:
                    kvarn_workspaces = (
                        ("dense", self._kvarn_dense_cache),
                        ("direct-stage", self._kvarn_direct_stage),
                        ("selected-fp8", self._kvarn_selected_stage),
                    )
                    for workspace_name, workspace in kvarn_workspaces:
                        if (
                            workspace is not None
                            and workspace.untyped_storage().data_ptr() == direct_storage
                        ):
                            raise RuntimeError(
                                "Direct packed scratch aliases the "
                                f"{workspace_name} KVarN workspace"
                            )
                    self._kvarn_validated_direct_scratch_storages.add(direct_storage)
                self._kvarn_diag_valid_counts = binding.nsa_cache_seqlens_int32
                exact_pool_only = complete_exact_pool

                static_inputs_validated = (
                    self._kvarn_native_packed_decode is not None
                    and num_actual_toks in self._kvarn_validated_direct_rows
                )
                out, lse = direct_packed_kvarn_mla_decode(
                    ...
                    sm_scale=self.scale,
                    candidate_envelope=(attn_metadata.kvarn_direct_candidate_envelope),
                    config=self._kvarn_config,
                    native_decode=self._kvarn_native_packed_decode,
                    static_inputs_validated=static_inputs_validated,
                    exact_pool_only=exact_pool_only,
                    fuse_kvarn_hadamard=fuse_kvarn_hadamard,
                )
                if (
                    self._kvarn_native_packed_decode is not None
                    and not static_inputs_validated
                ):
                    self._kvarn_validated_direct_rows.add(num_actual_toks)
                return (
                    (
                        out
                        if fuse_kvarn_hadamard
                        else self._restore_kvarn_mla_output(out)
                    ),
                    lse if self.need_to_return_lse_for_decode else None,
                )
            if use_decode_kernel:
                if self._kvarn_bf16_decode_diagnostic:
                    dense_cache, dense_indices = self._materialize_kvarn_mla_cache(
                        selected_indices,
                        attn_metadata,
                        kv_c_and_k_pe_cache,
                    )
                ...
                self._kvarn_diag_selected_indices = selected_indices
                self._kvarn_diag_valid_counts = attn_metadata.nsa_cache_seqlens
                kv_cache, selected_indices = self._stage_selected_kvarn_mla_fp8_cache(
                    selected_indices,
                    kv_c_and_k_pe_cache,
                )
            else:
