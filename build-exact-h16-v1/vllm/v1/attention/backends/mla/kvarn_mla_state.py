    ...
    def update(
        ...
        for req_id, num_scheduled_tokens in scheduled_tokens.items():
            ...
            for (
                ...
                full_context = precision_tail_tokens >= max_end_tokens
                if full_context:
...
class KVarNMLAStateManager:
    ...
    def prepare_step(
        ...
        block_fills: Mapping[int, int | None],
        ...
        if state is None:
            ...
            impls = tuple(
                impl
                for impl in cls._impls
                if impl._is_kvarn_mla and impl.layer_name in layer_name_set
            )
            if not impls:
                return
            ...
            state = _GroupState(
                pool_size=pool_size,
                packed_format=config.packed_format,
                impls=impls,
            )
            ...
            if state.packed_format != config.packed_format:
                raise RuntimeError(
                    "KVarN MLA ownership group cannot mix packed formats: "
                    f"{state.packed_format!r} and {config.packed_format!r}"
                )
            impls = state.impls

        needed = block_fills
        if len(needed) > state.pool_size:
            raise KVarNMLAExactPoolPressure(
                "MLA KVarN exact-block pool pressure: "
                f"need {len(needed)} live slots, pool has {state.pool_size}"
            )
        for block_id, fill in needed.items():
            if fill is None:
        ...
        missing = sorted(
            block_id for block_id in needed if block_id not in state.mapping
        )
