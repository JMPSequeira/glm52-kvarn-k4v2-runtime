...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dcp_world_size = (
            1
            if getattr(self.kv_cache_spec, "dcp_replicated", False)
            else int(
                getattr(self.kv_cache_spec, "dcp_kv_shard_count", None)
                or configured_dcp_world_size
            )
        )
    ...
    def build(
        ...
        if num_decodes > 0:
            ...
            has_token_padding = (
                num_decode_tokens != int(decode_lens_cpu.sum().item())
            )
            use_native = (
                not (self.use_flattening or use_varlen or has_token_padding)
            ...
            seq_lens, block_table, decode_lens, batch_size, requires_padding = (
                self._prepare_decode_tensors(
                    ...
                    next_n=next_n,
                    max_decode_len=max_decode_len,
                    force_flatten=use_varlen or has_token_padding,
                )
            )

            seq_lens_is_buffer_view = (use_native and next_n > 1) or (
                not use_native
                and (max_decode_len > 1 or use_varlen or has_token_padding)
            )
