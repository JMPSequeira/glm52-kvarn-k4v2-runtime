...
class FlexAttentionMetadata:
    ...
    # DSpark keeps a causal synthetic prefix while anchoring the context
    # window to the first query token in each parallel draft block.
    dspark_anchor_window: int | None = None
    ...
    def get_dspark_anchor_mask_mod(self) -> _mask_mod_signature:
        """Create the exact Speculators anchored causal-window mask."""
        assert self.doc_ids is not None
        assert self.dspark_anchor_window is not None
        window = self.dspark_anchor_window
        ...
        def final_mask_mod(
            ...
            # The sliding-window KV manager frees whole blocks covering
            # tokens [0, processed - W + 1) with processed <= decode_offset,
            # so token A - W can live in a freed (and recycled) page whose
            # runner-side block-table entry is stale. Clamp the mask to the
            # eviction boundary (A - W + 1) so the draft never attends a
            # freed page; at contexts <= W this clamps below zero and the
            # full context stays visible, exactly as before.
            first_context = self.decode_offset[q_req] - window + 1
            return (
                is_valid
                & (logical_kv_idx <= logical_q_idx)
                & (logical_kv_idx >= first_context)
            )
    ...
    def get_mask_mod(self):
        ...
        if self.dspark_anchor_window is not None:
            mask_mod = self.get_dspark_anchor_mask_mod()
        return mask_mod
    ...
    def _build_block_mask_direct(self) -> BlockMask:
        ...
        use_dspark_anchor = self.dspark_anchor_window is not None
        needs_per_q_pruning = (
            self.causal
            or self.sliding_window
            or custom_hint
            or use_rswa
            or use_dspark_anchor
        )
        ...
        if needs_per_q_pruning:
            ...
            if use_dspark_anchor:
                assert self.dspark_anchor_window is not None
                anchor = self.decode_offset[self.doc_ids]
                # +1 keeps the pruned block set consistent with the anchor
                # mask's eviction-boundary clamp (see get_dspark_anchor_mask_mod):
                # never gather a whole-freed sliding-window block.
                min_kv_idx = torch.clamp(
                    anchor - self.dspark_anchor_window + 1, min=0
                )
                min_block_idx = min_kv_idx // self.block_size
                anchored_mask = self.logical_block_ids >= min_block_idx[:, None]
                used_pages.masked_fill_(~anchored_mask, 0)
            if use_rswa:
...
class FlexAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if (
            ...
            attn_metadata.mask_mod = attn_metadata.get_mask_mod()
            needs_rebuild_block_mask = True
        dspark_anchor_window = getattr(layer, "dspark_anchor_window", None)
        if attn_metadata.dspark_anchor_window != dspark_anchor_window:
            attn_metadata.dspark_anchor_window = dspark_anchor_window
            attn_metadata.mask_mod = attn_metadata.get_mask_mod()
            needs_rebuild_block_mask = True
