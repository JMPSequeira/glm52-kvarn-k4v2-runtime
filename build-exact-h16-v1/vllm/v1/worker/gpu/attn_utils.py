...
def get_kvarn_mla_precision_tail_tokens(kv_cache_config: KVCacheConfig) -> int:
    """Return the shared target precision tail used for graph specialization."""
    precision_tails: set[int] = set()
    for group in kv_cache_config.kv_cache_groups:
        ...
        precision_tails.add(
            KVarNMLAConfig.from_cache_dtype(spec.cache_dtype_str).precision_tail_tokens
        )
    if len(precision_tails) > 1:
        raise RuntimeError(
            "KVarN MLA graph specialization requires one precision-tail size "
            f"across cache groups, got {sorted(precision_tails)}"
        )
    return next(iter(precision_tails), 0)
...
def init_kvarn_mla_live_block_trackers(
    ...
    dcp_rank: int,
    cp_interleave: int,
    precision_tail_tokens: int | None = None,
) -> dict[int, Any]:
    ...
    for group_id, group in enumerate(kv_cache_config.kv_cache_groups):
        ...
        group_configs[group_id] = (
            ...
            (
                config.precision_tail_tokens
                if precision_tail_tokens is None
                else precision_tail_tokens
            ),
