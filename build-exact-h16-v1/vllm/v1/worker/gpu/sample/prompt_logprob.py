...
from vllm.v1.worker.gpu.item13_prompt_logits import Item13PromptLogitsCapture


_PROMPT_LOGPROBS_CHUNK_SIZE_ENV = "VLLM_PROMPT_LOGPROBS_CHUNK_SIZE"
_DEFAULT_PROMPT_LOGPROBS_CHUNK_SIZE = 1024


def _prompt_logprobs_chunk_size() -> int:
    raw_value = os.getenv(
        _PROMPT_LOGPROBS_CHUNK_SIZE_ENV,
        str(_DEFAULT_PROMPT_LOGPROBS_CHUNK_SIZE),
    )
    try:
        chunk_size = int(raw_value)
    except ValueError as error:
        raise ValueError(
            f"{_PROMPT_LOGPROBS_CHUNK_SIZE_ENV} must be a positive integer, "
            f"got {raw_value!r}"
        ) from error
    if chunk_size <= 0:
        raise ValueError(
...
class PromptLogprobsWorker:
    def __init__(self, max_num_reqs: int):
        ...
        self.raw_logits_capture = Item13PromptLogitsCapture.from_env()
    ...
    def compute_prompt_logprobs(
        ...
        capture = self.raw_logits_capture
        capture_req_id: str | None = None
        raw_logits_callback: Callable[[torch.Tensor, torch.Tensor], None] | None = None
        if capture is not None:
            has_planned_request = any(
                capture.is_planned_request_id(req_id)
                for req_id in input_batch.req_ids
            )
            if has_planned_request:
                capture_indices = np.flatnonzero(needs_prompt_logprobs)
                if len(input_batch.req_ids) != 1 or capture_indices.tolist() != [0]:
                    raise RuntimeError(
                        "item-13 raw-logit capture requires one serial "
                        "prompt-logprob request"
                    )
                capture_req_id = input_batch.req_ids[0]
                capture.begin(capture_req_id)

                def raw_logits_callback(
                    logits: torch.Tensor, label_token_ids: torch.Tensor
                ) -> None:
                    assert capture_req_id is not None
                    capture.append(capture_req_id, logits, label_token_ids)
        ...
        if capture_req_id is not None:
            capture.finish(
                capture_req_id,
                final_prompt_chunk=not bool(is_prompt_chunked[0]),
            )
...
def compute_prompt_logprobs_with_chunking(
    ...
    chunk_size = _prompt_logprobs_chunk_size()
    token_ids = []
    logprobs = []
    ranks = []
    ...
    for start_idx in range(0, prompt_token_ids.shape[0], chunk_size):
        ...
        if raw_logits_callback is not None:
            raw_logits_callback(
                prompt_logits,
