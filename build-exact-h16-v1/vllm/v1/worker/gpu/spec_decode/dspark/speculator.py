...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Default-off n-gram assist: host-side per-slot draft-token
        # override from a prompt+accepted-prefix k-gram index. All knobs
        # are read through getattr with the same defaults as vllm/envs.py
        # so this file also runs unchanged against an envs.py without the
        # knobs (e.g. the pinned control overlay set); knob-off runs never
        # import the assist module.
        self.ngram_assist = None
        if getattr(envs, "VLLM_DSPARK_NGRAM_ASSIST", False):
            from vllm.v1.worker.gpu.spec_decode.dspark.ngram_assist import (
                NgramAssist,
            )

            self.ngram_assist = NgramAssist(
                k=int(getattr(envs, "VLLM_DSPARK_NGRAM_K", 5)),
                max_batch=int(getattr(envs, "VLLM_DSPARK_NGRAM_MAX_BATCH", 64)),
                max_index_tokens=int(
                    getattr(envs, "VLLM_DSPARK_NGRAM_MAX_INDEX_TOKENS", 65536)
                ),
                log_path=str(getattr(envs, "VLLM_DSPARK_NGRAM_ASSIST_LOG", "")),
                device=device,
                max_num_reqs=self.max_num_reqs,
                num_speculative_steps=self.num_speculative_steps,
                debug_iters=int(getattr(envs, "VLLM_DSPARK_NGRAM_DEBUG", 0)),
            )
    ...
    def propose(
        ...
        **kwargs,
        ...
        if self.ngram_assist is not None:
            # Stage the token-stream length readback BEFORE the captured
            # draft graph replays (adaptive-depth composition pattern:
            # host state lives outside the graph).
            self.ngram_assist.begin_step(input_batch)
        ...
        if self.ngram_assist is not None:
            # Host-side per-slot override AFTER graph replay: the FULL
            # draft graph is untouched; fail-closed inside.
            draft_tokens = self.ngram_assist.apply(
                input_batch,
                draft_tokens,
    ...
    def set_token_stream(self, all_token_ids, total_len: torch.Tensor) -> None:
        """Runner hook (hasattr-guarded): hand the assist the authoritative
        per-request token stream (UVA-backed [max_num_reqs, max_model_len]
        ids written by post_update) and the per-slot stream lengths. No-op
        unless VLLM_DSPARK_NGRAM_ASSIST is enabled."""
        if self.ngram_assist is not None:
            self.ngram_assist.set_token_stream(all_token_ids, total_len)
