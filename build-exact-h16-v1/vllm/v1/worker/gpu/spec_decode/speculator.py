...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Pinned so CUDA-graph capture paths that stage CPU-side batch
        # topology (e.g. the FixedMTP3 proposer supergraph, which builds
        # draft attention metadata INSIDE the capture region) can copy it
        # host->device legally. Contents are write-once.
        self.arange = torch.arange(
            self.max_num_reqs + 1, dtype=torch.int32, device="cpu", pin_memory=True
        )
        # Pinned uniform query_start_loc staging: written in place per call
        # so no fresh unpinned CPU tensor is created inside a capture.
        # Draft decode consumers read it host-side (numpy batch topology);
        # a single stable pinned buffer preserves replay semantics.
        self.uniform_query_start_loc_cpu = torch.zeros(
            self.max_num_reqs + 1, dtype=torch.int32, device="cpu", pin_memory=True
        )
    ...
    def set_attn(
        ...
        dcp_replicated_draft = block_tables.cp_size > 1 and any(
        ...
        kvarn_mla_draft = any(
            getattr(attn_group.get_metadata_builder(0), "kvarn_config", None)
            is not None
            for attn_group_set in self.attn_groups
            for attn_group in attn_group_set
        )
        self.rebuild_prefill_attn_metadata = (
            dcp_replicated_draft or kvarn_mla_draft
        )
    ...
    def _build_draft_attn_metadata(
        ...
        if query_start_loc_cpu is None:
            ...
            # Written IN PLACE into the pinned staging buffer: no fresh
            # unpinned CPU tensor inside a CUDA graph capture region (the
            # FixedMTP3 supergraph builds draft attention metadata while
            # capturing), and any host->device staging of it by an attention
            # metadata builder is capture-legal (pinned source).
            query_start_loc_cpu = self.uniform_query_start_loc_cpu[
                : num_reqs_padded + 1
            ]
            torch.clamp(
                self.arange[: num_reqs_padded + 1], max=num_reqs, out=query_start_loc_cpu
            )
            query_start_loc_cpu.mul_(num_query_per_req)
    ...
    def _copy_request_inputs(
        ...
        if self.draft_logits is not None:
            # Temperature and seeds are only consumed by probabilistic draft
            # sampling. Avoid two device copies on the exact-greedy path.
            # NOTE(woosuk): For draft sampling, we only consider the temperature
            # and ignore the other sampling parameters such as top_k and top_p,
            # for simplicity and performance. While this may slightly degrade the
            # acceptance rate, it does not affect the output distribution after
            # rejection sampling.
