...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def capture(self) -> None:
        ...
        if self.num_speculative_steps == 1:
            return
        if getattr(self, "fixed_mtp3_supergraph", False):
            manager = self.decode_cudagraph_manager
            if not isinstance(manager, FixedMTP3CudaGraphManager):
                raise RuntimeError(
                    "Fixed MTP3 proposer graph manager is not initialized."
                )
            manager.capture_fixed_mtp3(
                self._fixed_mtp3_decode,
                self.model_state,
                self.input_buffers,
                self.block_tables,
                self.attn_groups,
    ...
    def _fixed_mtp3_replay_enabled(
        self,
        *,
        dummy_run: bool,
        is_profile: bool,
        ...
        if not getattr(self, "fixed_mtp3_supergraph", False):
            return False
        if dummy_run or is_profile:
            # Startup profiling and compile warmups run before the recurrent
            # graph is captured. They may exercise the existing eager proposer;
            # real requests below remain fail-closed on graph ownership.
            return False
        manager = self.decode_cudagraph_manager
        if not isinstance(manager, FixedMTP3CudaGraphManager):
            raise RuntimeError(
                "Fixed MTP3 requires its captured proposer graph manager; "
                "baseline fallback is disabled."
            )
        if not manager._graphs_captured or not manager.fixed_mtp3_graphs:
            raise RuntimeError(
                "Fixed MTP3 proposer supergraphs are not captured; eager "
                "or per-step fallback is disabled."
            )
        self._validate_fixed_mtp3_runtime(
    ...
    def propose(
        ...
        fixed_mtp3_replay = self._fixed_mtp3_replay_enabled(
            dummy_run=dummy_run,
        ...
        if fixed_mtp3_replay:
            with record_function_or_nullcontext(
                "vllm:v2/speculator/decode/fixed_mtp3_supergraph_replay"
            ):
                self._run_fixed_mtp3_supergraph(num_reqs)
        ...
        with record_function_or_nullcontext("vllm:v2/speculator/decode/dispatch"):
            decode_batch_desc, num_tokens_across_dp = dispatch_cg_and_sync_dp(
                ...
                need_eager=is_profile
                or (
                    getattr(self, "fixed_mtp3_supergraph", False)
                    and (dummy_run or is_profile)
                ),
            )
