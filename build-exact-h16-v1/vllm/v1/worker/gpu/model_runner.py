...
from vllm.model_executor.layers.quantization.kvarn.config import (
    is_deepseek_mtp_draft_model_config,
)
...
from vllm.v1.worker.gpu.attn_utils import (
    ...
    get_kv_cache_spec,
    get_kvarn_mla_precision_tail_tokens,
    init_attn_backend,
    init_kv_cache,
...
class _KVarNMLARequestState:
    ...
    ownership_rollback_tokens: int = 0

    @property
    def tracker_key(self) -> str:
        return f"{self.req_id}:{self.generation}"

def _kvarn_mla_exact_graph_class(
    requests: Mapping[str, _KVarNMLARequestState],
    scheduled_tokens: Mapping[str, int],
    precision_tail_tokens: int,
) -> bool:
    """Classify target attention from scheduler-owned context lengths."""
    if precision_tail_tokens <= 0 or not scheduled_tokens:
        return False
    for req_id, num_scheduled_tokens in scheduled_tokens.items():
        request = requests.get(req_id)
        if request is None:
            raise RuntimeError(
                f"MLA KVarN exact graph classification missing request {req_id!r}"
            )
        if num_scheduled_tokens < 0:
            raise RuntimeError(
                "MLA KVarN exact graph classification received a negative "
                f"scheduled-token count for {req_id!r}"
            )
        if request.num_computed_tokens + num_scheduled_tokens > precision_tail_tokens:
            return False
    return True

@dataclass
class _KVarNMLAOwnershipSnapshot:
    req_id: str
    tracker_key: str
    generation: int
    ownership_step: int
    ...
    rollback_tokens: int
    output_index: int = 0
    actual_end_tokens: int | None = None
    applied: bool = False


@dataclass
class _KVarNMLAOwnershipEpoch:
    cache_generation: int
    snapshots: list[_KVarNMLAOwnershipSnapshot]
    completion_ends: dict[str, int] | None = None
    resolve_completion: Callable[[], None] | None = None
    discard_completion: Callable[[], None] | None = None
    lock: Lock = field(default_factory=Lock)

    def publish_completion(self, num_rejected_tokens: np.ndarray) -> None:
        with self.lock:
            required_rows = 1 + max(
                snapshot.output_index for snapshot in self.snapshots
            )
            if len(num_rejected_tokens) < required_rows:
                raise RuntimeError(
                    "MLA KVarN ownership completion size mismatch: "
                    f"expected at least {required_rows}, got "
                    f"{len(num_rejected_tokens)}"
                )
            if self.completion_ends is not None:
                return
            self.completion_ends = {
                snapshot.tracker_key: (
                    snapshot.start_tokens
                    + snapshot.scheduled_tokens
                    - int(num_rejected_tokens[snapshot.output_index])
                )
                for snapshot in self.snapshots
            }

    def take_completion_ends(self) -> dict[str, int] | None:
        with self.lock:
            completion_ends = self.completion_ends
            self.completion_ends = None
            return completion_ends
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self._kvarn_mla_mtp_live_block_trackers: dict[int, Any] = {}
        ...
        self._kvarn_mla_ownership_epochs: deque[_KVarNMLAOwnershipEpoch] = deque()
        self._kvarn_mla_cache_generation = 0
    ...
    def _reset_kv_cache_binding_state(self) -> None:
        """Invalidate all state derived from the prior physical KV cache."""
        if self._kvarn_mla_ownership_epochs:
            self._force_kvarn_mla_ownership_resolution(
                "KV-cache rebinding", refresh=False
            )
        self._kvarn_mla_live_block_trackers.clear()
        self._kvarn_mla_mtp_live_block_trackers.clear()
        self._kvarn_mla_requests.clear()
        self._kvarn_mla_generations.clear()
    ...
    def _init_kvarn_mla_mtp_live_block_trackers(
        self, kv_cache_config: KVCacheConfig
    ) -> dict[int, Any]:
        speculator = self.speculator
        draft_model_config = getattr(speculator, "draft_model_config", None)
        if (
            not isinstance(speculator, DraftModelSpeculator)
            or not is_deepseek_mtp_draft_model_config(draft_model_config)
        ):
            return {}
        trackers = init_kvarn_mla_live_block_trackers(
            ...
            self.dcp_rank,
            self.cp_interleave,
            precision_tail_tokens=int(draft_model_config.max_model_len),
        )
        if trackers:
            speculator.rebuild_prefill_attn_metadata = True
        return trackers
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        native_kvarn_mla_exact = any(
            bool(
                getattr(
                    getattr(layer, "impl", None),
                    "_kvarn_direct_packed_decode",
                    False,
                )
            )
            and getattr(
                getattr(layer, "impl", None), "_kvarn_native_packed_decode", None
            )
            is not None
            for layer in self.compilation_config.static_forward_context.values()
        )
        self._kvarn_mla_exact_cudagraph_enabled = bool(
            native_kvarn_mla_exact and envs.VLLM_KVARN_MLA_EXACT_CUDAGRAPH
        )
        self._kvarn_mla_precision_tail_tokens = (
            get_kvarn_mla_precision_tail_tokens(self.kv_cache_config)
            if self._kvarn_mla_exact_cudagraph_enabled
            else 0
        )
        self.cudagraph_manager = ModelCudaGraphManager(
            ...
            kvarn_mla_precision_tail_tokens=(
                self._kvarn_mla_precision_tail_tokens
            ),
        )
        ...
        if isinstance(self.speculator, DraftModelSpeculator):
            with use_workspace_lane(1):
                ...
                if hasattr(self.speculator, "set_token_stream"):
                    # DSpark n-gram assist (default-off): authoritative
                    # per-request token stream + lengths for the host-side
                    # draft override. No-op unless the assist is enabled.
                    self.speculator.set_token_stream(
        ...
        self._kvarn_mla_mtp_live_block_trackers = (
            self._init_kvarn_mla_mtp_live_block_trackers(kv_cache_config)
        )
        self._kvarn_mla_requests.clear()
        self._kvarn_mla_removed_tracker_keys.clear()
        self._refresh_kvarn_mla_block_fills()
    ...
    def _iter_kvarn_mla_live_block_trackers(self):
        yield from self._kvarn_mla_live_block_trackers.values()
        yield from getattr(
            self, "_kvarn_mla_mtp_live_block_trackers", {}
        ).values()

    def _get_kvarn_mla_block_fills(
        self, trackers: dict[int, Any]
    ) -> KVarNMLABlockFills | None:
        if not trackers:
            return None
        block_fills: list[dict[int, int | None] | None] = [
            None
    ...
    @staticmethod
    def _prepare_kvarn_mla_ownership(
        attn_groups: list[list[Any]],
        block_fills: KVarNMLABlockFills | None,
        excluded_layer_names: set[str] | None = None,
    ) -> None:
        if block_fills is None:
            return
        for group_id, group_block_fills in enumerate(block_fills):
            if group_block_fills is None:
                continue
            for attn_group in attn_groups[group_id]:
                builder = attn_group.get_metadata_builder(0)
                prepare_ownership = getattr(
                    builder, "prepare_kvarn_mla_ownership", None
                )
                if prepare_ownership is not None:
                    prepare_ownership(
                        group_block_fills,
                        excluded_layer_names=excluded_layer_names,
                    )

    def _get_kvarn_mla_ownership_snapshots(
        self,
    ) -> tuple[KVarNMLABlockFills | None, KVarNMLABlockFills | None]:
        return (
            self._get_kvarn_mla_block_fills(
                self._kvarn_mla_live_block_trackers
            ),
            self._get_kvarn_mla_block_fills(
                self._kvarn_mla_mtp_live_block_trackers
            ),
    ...
    def _apply_kvarn_mla_ownership_snapshots(
        self,
        target: KVarNMLABlockFills | None,
        mtp: KVarNMLABlockFills | None,
    ) -> None:
        draft_layer_names = (
        ...
        self._prepare_kvarn_mla_ownership(
            self.attn_groups,
            target,
            excluded_layer_names=draft_layer_names,
        )
        if isinstance(self.speculator, DraftModelSpeculator):
            self._prepare_kvarn_mla_ownership(self.speculator.attn_groups, mtp)

    def _refresh_kvarn_mla_block_fills(self) -> None:
        target, mtp = self._get_kvarn_mla_ownership_snapshots()
        try:
            self._apply_kvarn_mla_ownership_snapshots(target, mtp)
        except KVarNMLAExactPoolPressure:
            # Retained epochs are the only reclaimable ownership here. Settle
            # them before retrying; a second pressure failure propagates.
            self._force_kvarn_mla_ownership_resolution(
                "exact-pool pressure", refresh=False
            )
            target, mtp = self._get_kvarn_mla_ownership_snapshots()
            self._apply_kvarn_mla_ownership_snapshots(target, mtp)

    def _drain_kvarn_mla_ownership_epochs(self) -> None:
        """Apply completed ownership epochs in submission order."""
        while self._kvarn_mla_ownership_epochs:
            epoch = self._kvarn_mla_ownership_epochs[0]
            completion_ends = epoch.take_completion_ends()
            if completion_ends is not None:
                for snapshot in epoch.snapshots:
                    if snapshot.actual_end_tokens is None:
                        snapshot.actual_end_tokens = completion_ends.get(
                            snapshot.tracker_key
                        )

            if not all(snapshot.applied for snapshot in epoch.snapshots):
                # An epoch becomes applicable either when its local output copy
                # completes naturally or when the next scheduler output proves
                # central completion. Partial epochs never pass one another.
                ready = all(
                    snapshot.applied or snapshot.actual_end_tokens is not None
                    for snapshot in epoch.snapshots
                )
                if not ready:
                    break
                for snapshot in epoch.snapshots:
                    if snapshot.applied:
                        continue
                    request = self._kvarn_mla_requests.get(snapshot.req_id)
                    current_generation = self._kvarn_mla_generations.get(
                        snapshot.req_id
                    )
                    if (
                        epoch.cache_generation == self._kvarn_mla_cache_generation
                        and current_generation == snapshot.generation
                        and request is not None
                        and request.tracker_key == snapshot.tracker_key
                        and request.ownership_step == snapshot.ownership_step
                    ):
                        assert snapshot.actual_end_tokens is not None
                        for tracker in self._iter_kvarn_mla_live_block_trackers():
                            tracker.resolve_async(
                                snapshot.epoch_id,
                                request,
                                snapshot.actual_end_tokens,
                            )
                        request.rollback_tokens = max(
                            request.rollback_tokens - snapshot.rollback_tokens,
                            0,
                        )
                    else:
                        # A removed or replaced generation remains owned until
                        # its epoch settles, then retires without stale fills.
                        for tracker in self._iter_kvarn_mla_live_block_trackers():
                            tracker.discard_epoch(snapshot.epoch_id)
                        self._kvarn_mla_removed_tracker_keys.add(
                            snapshot.tracker_key
                        )
                    snapshot.applied = True

            self._kvarn_mla_ownership_epochs.popleft()
            if epoch.discard_completion is not None:
                epoch.discard_completion()



    def _force_kvarn_mla_ownership_resolution(
        self, reason: str, *, refresh: bool = True
    ) -> None:
        """Synchronize outstanding epochs only at a correctness pressure point."""
        while self._kvarn_mla_ownership_epochs:
            epoch = self._kvarn_mla_ownership_epochs[0]
            resolve_completion = epoch.resolve_completion
            if resolve_completion is None:
                raise RuntimeError(
                    f"MLA KVarN cannot resolve ownership during {reason}"
                )
            resolve_completion()
            self._drain_kvarn_mla_ownership_epochs()
            if self._kvarn_mla_ownership_epochs and (
                self._kvarn_mla_ownership_epochs[0] is epoch
            ):
                raise RuntimeError(
                    f"MLA KVarN ownership did not settle during {reason}"
                )
        if self._kvarn_mla_removed_tracker_keys:
            ...
            for tracker in self._iter_kvarn_mla_live_block_trackers():
                tracker.update(requests, {}, removed, ())
        if refresh:
            self._refresh_kvarn_mla_block_fills()

    def _update_kvarn_mla_ownership(
        ...
        for req_id, num_scheduled_tokens in (
            ...
            unresolved_rollback = request.rollback_tokens + len(
                scheduled_drafts.get(req_id, ())
            )
            rollback_tokens[request.tracker_key] = unresolved_rollback
            if unresolved_rollback:
                ownership_epochs[request.tracker_key] = (
                    f"{request.tracker_key}@{request.ownership_step + 1}"
                )
        ...
        for tracker in self._iter_kvarn_mla_live_block_trackers():
            tracker.update(
                ...
                (),
                rollback_tokens,
                ownership_epochs=ownership_epochs,
            )

        for req_id in scheduler_output.num_scheduled_tokens:
            request = self._kvarn_mla_requests[req_id]
            rollback = len(scheduled_drafts.get(req_id, ()))
            request.rollback_tokens += rollback
            request.ownership_rollback_tokens = rollback
    ...
    def _make_kvarn_mla_ownership_epoch(
        self, req_ids: list[str]
    ) -> _KVarNMLAOwnershipEpoch | None:
        if not self._kvarn_mla_live_block_trackers or self.num_speculative_steps == 0:
            return None
        snapshots = []
        for output_index, req_id in enumerate(req_ids):
            request = self._kvarn_mla_requests[req_id]
            if request.ownership_rollback_tokens == 0:
                continue
            snapshots.append(
                _KVarNMLAOwnershipSnapshot(
                    req_id=req_id,
                    tracker_key=request.tracker_key,
                    generation=request.generation,
                    ownership_step=request.ownership_step,
                    start_tokens=request.ownership_start_tokens,
                    scheduled_tokens=request.ownership_scheduled_tokens,
                    epoch_id=(
                        f"{request.tracker_key}@{request.ownership_step}"
                    ),
                    output_index=output_index,
                    rollback_tokens=request.ownership_rollback_tokens,
                )
            )
        if not snapshots:
            return None
        return _KVarNMLAOwnershipEpoch(
            cache_generation=self._kvarn_mla_cache_generation,
            snapshots=snapshots,
        )
    ...
    def _remove_request(self, req_id: str) -> bool:
        ...
        if kvarn_request is not None:
            has_unresolved_epoch = any(
                any(
                    not snapshot.applied
                    and snapshot.tracker_key == kvarn_request.tracker_key
                    for snapshot in epoch.snapshots
                )
                for epoch in self._kvarn_mla_ownership_epochs
            )
            if not has_unresolved_epoch:
                self._kvarn_mla_removed_tracker_keys.add(
                    kvarn_request.tracker_key
                )
            else:
                for tracker in self._iter_kvarn_mla_live_block_trackers():
    ...
    def execute_model(
        ...
        kvarn_mla_exact = False if dummy_run else _kvarn_mla_exact_graph_class(
            self._kvarn_mla_requests,
            scheduler_output.num_scheduled_tokens,
            getattr(self, "_kvarn_mla_precision_tail_tokens", 0),
        )
        ...
        with record_function_or_nullcontext("vllm:v2/target/dispatch"):
            batch_desc, num_tokens_across_dp = dispatch_cg_and_sync_dp(
                ...
                kvarn_mla_exact=kvarn_mla_exact,
            )
        if use_varlen_capacity:
            assert verification_capacity_manager is not None
        ...
        if not (dummy_run and skip_attn_for_dummy_run):
            ...
            with record_function_or_nullcontext(
                ...
                kvarn_mla_metadata_exact = (
                    batch_desc.kvarn_mla_exact
                    if self._kvarn_mla_exact_cudagraph_enabled
                    else None
                )
                attn_metadata = self.model_state.prepare_attn(
                    ...
                    self.attn_groups,
                    self.kv_cache_config,
                    kvarn_mla_exact=kvarn_mla_metadata_exact,
                )
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            batch_descriptor = BatchDescriptor(
                ...
                kvarn_mla_exact=batch_desc.kvarn_mla_exact,
            )
    ...
    def sample_tokens(
        ...
        ownership_epoch = self._make_kvarn_mla_ownership_epoch(
            input_batch.req_ids
        )
        ownership_callback = (
            None
            if ownership_epoch is None
            else lambda _num_sampled, num_rejected: (
                ownership_epoch.publish_completion(num_rejected)
            )
        )
        ...
        if ownership_epoch is not None:
            ownership_epoch.resolve_completion = async_output.resolve_completion
            ownership_epoch.discard_completion = (
                async_output.discard_completion_callback
            )
            self._kvarn_mla_ownership_epochs.append(ownership_epoch)
        ...
        if self.speculator is not None:
            ...
            with (
                ...
                if self._kvarn_mla_mtp_live_block_trackers:
            ...
            with record_function_or_nullcontext(
                ...
                if num_draft_tokens > 0:
                    ...
                    # The proposer tensor already has request-batch ordering and
                    # the same values just stored above. Re-index only when the
                    # handler will asynchronously read real tokens: that path
                    # needs a temporary allocation whose lifetime it can extend
                    # with record_stream(). Fixed-width placeholder handling only
                    # consumes the shape, while AsyncModelRunnerOutput snapshots
                    # its input synchronously before starting its host copy.
                    if (
                        not copy_draft_with_output
                        and self.draft_tokens_handler.needs_host_copy(input_batch)
                    ):
