...
def _is_compatible(
    ...
    return (
        ...
        and desc.kvarn_mla_exact == kvarn_mla_exact
    )
...
class CudaGraphManager:
    def __init__(
        ...
        kvarn_mla_precision_tail_tokens: int = 0,
    ):
        ...
        self.kvarn_mla_precision_tail_tokens = kvarn_mla_precision_tail_tokens
        self.kvarn_mla_graph_cases = (
            (False, True) if kvarn_mla_precision_tail_tokens > 0 else (False,)
        )
    ...
    def _init_candidates(self) -> None:
        ...
        if not (self.cudagraph_mode and capture_sizes):
            return
        kvarn_mla_graph_cases = getattr(self, "kvarn_mla_graph_cases", (False,))
        ...
        for num_tokens, num_active_loras, kvarn_mla_exact in product(
            capture_sizes, self.lora_capture_cases, kvarn_mla_graph_cases
        ):
            if separate_decode_routine and decode_mode:
                for desc in decode_descs(
                    num_tokens, num_active_loras, kvarn_mla_exact
                ):
                    if desc in descs_by_mode[desc.cg_mode]:
                        continue
                    descs_by_mode[desc.cg_mode].append(desc)
                    descs_by_token_lora[
                        (desc.num_tokens, num_active_loras, kvarn_mla_exact)
                    ].append(desc)
        ...
        if (
            ...
            for num_reqs in range(1, max_uniform_reqs + 1):
                ...
                if num_tokens > max_decode_tokens or num_tokens > max_cg_capture_size:
                    continue
                for num_active_loras, kvarn_mla_exact in product(
                    self.lora_capture_cases, kvarn_mla_graph_cases
                ):
                    desc = BatchExecutionDescriptor(
                        cg_mode=decode_mode,
                        num_tokens=num_tokens,
            ...
            for i in range(current_range_start, token_cg_size + 1):
                for num_active_loras, kvarn_mla_exact in product(
                    ...
                    if staging_key in descs_by_token_lora:
                        self._candidates[
                            (i, num_active_loras, kvarn_mla_exact)
    ...
    def dispatch(
        ...
        key = (num_tokens, effective_loras, kvarn_mla_exact)
...
class ModelCudaGraphManager(CudaGraphManager):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            kvarn_mla_precision_tail_tokens=kvarn_mla_precision_tail_tokens,
        )
    ...
    def capture(
        ...
        def create_forward_fn(
            ...
            attn_metadata, slot_mappings = prepare_inputs_to_capture(
                ...
                kvarn_mla_exact=desc.kvarn_mla_exact,
            )
