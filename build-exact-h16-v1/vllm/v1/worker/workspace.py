...
def use_workspace_lane(lane: int) -> Iterator[None]:
    """Select the target or speculative workspace role.

    Lanes own separate buffers unless ``VLLM_SHARE_TARGET_DRAFT_WORKSPACE=1``.
    The shared mode is valid only when target and speculative execution are
    serialized and DBO is disabled.
    """
    if lane < 0:
...
class WorkspaceManager:
    ...
    Owns reusable buffers per ubatch and, by default, per lane. A serialized
    target/speculative deployment may explicitly share lane storage.
    """
    ...
    def __init__(
        ...
        self._share_lanes = (
            os.environ.get("VLLM_SHARE_TARGET_DRAFT_WORKSPACE", "0") == "1"
        )
        if self._share_lanes and self._num_ubatches != 1:
            raise ValueError("Shared target/draft workspace requires DBO off.")
    ...
    def _ensure_workspace_size(self, required_bytes: int) -> torch.Tensor:
        ...
        lane_offset = 0 if self._share_lanes else lane
        workspace_id = ubatch_id * self._num_lanes + lane_offset
