...
def resolve_kv_cache_block_sizes(
    ...
    group_block_sizes = [
        _effective_kv_cache_block_size(group.kv_cache_spec, dcp, pcp)
        for group in groups
    ]
...
def _get_kvarn_mla_workspace_config(
    ...
    for group in kv_cache_groups:
        ...
        kvarn_specs.extend(
            ...
            and spec.cache_dtype_str in ("kvarn_mla_k5_g64", "kvarn_nvfp4_ds_mla")
        )
    ...
    config = next(iter(geometries.values()))
    if geometry[0] != config.group or geometry[4] != config.packed_page_bytes:
        raise ValueError(
            "Unsupported KVarN MLA cache geometry: "
            f"block_size={geometry[0]}, page_size_bytes={geometry[4]}, "
            f"packed_format={config.packed_format}"
        )
    return config
...
def _get_kvarn_mla_num_blocks(
    ...
    """Solve packed-cache plus graph-static-workspace capacity for MLA KVarN."""
    ...
    if (
        ...
        raise ValueError(
            f"Invalid available KVarN MLA memory envelope: {available_memory!r}"
        )
    if (
        isinstance(packed_bytes_per_block, bool)
        or not isinstance(packed_bytes_per_block, int)
        or packed_bytes_per_block <= 0
    ):
        raise ValueError(
            f"Invalid KVarN MLA packed bytes per block: {packed_bytes_per_block!r}"
        )

    def required_memory(num_pages: int) -> int:
        if num_pages < 0:
            raise ValueError(f"KVarN MLA page count cannot be negative: {num_pages}")
        workspace_bytes = config.workspace_envelope(vllm_config, num_pages).total_bytes
        required_bytes = packed_bytes_per_block * num_pages + workspace_bytes
        if required_bytes > (1 << 63) - 1:
            raise OverflowError(
                "KVarN MLA packed cache plus workspace exceeds int64 bytes: "
                f"{required_bytes}"
            )
    ...
    if override is not None:
        if override < 0:
            raise ValueError(f"num_gpu_blocks_override cannot be negative: {override}")
        if override == 0:
            return 0
        if available_memory == 0:
            return override
        ...
        if required_bytes > available_memory:
            raise ValueError(
                ...
                "bytes for packed KVarN MLA cache pages and the graph-static "
                f"workspace, but only {available_memory} bytes are available"
            )
        return override

    max_pages_without_workspace = available_memory // packed_bytes_per_block
...
def _validate_partial_indexer_kvarn_capacity(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
    *,
    minimal_profile: bool = False,
) -> None:
    """Fail closed unless partial indexer storage preserves C1 capacity."""
    shards = int(envs.VLLM_DCP_INDEXER_SHARDS)
    dcp_size = vllm_config.parallel_config.decode_context_parallel_size
    if not 1 < shards < dcp_size:
        return
    if _get_kvarn_mla_workspace_config(kv_cache_groups) is None:
        return
    if not _use_lockstep_mla_allocation(
        kv_cache_groups,
        dcp_size,
        vllm_config.parallel_config.prefill_context_parallel_size,
    ):
        raise ValueError(
            "Partial sparse-indexer KVarN caches must share the main cache's "
            "logical block-ID namespace"
        )
    if minimal_profile:
        return
    override = vllm_config.cache_config.num_gpu_blocks_override
    if override != 641:
        raise ValueError(
            "Partial sparse-indexer allocation with KVarN must preserve the "
            "main cache at num_gpu_blocks_override=641; "
            f"got {override!r}"
        )
...
def _is_lockstep_mla_spec_layout(
    ...
    if not all(isinstance(spec, MLAAttentionSpec) for spec in specs):
        return False
    if not any(
        spec.cache_dtype_str in ("kvarn_mla_k5_g64", "kvarn_nvfp4_ds_mla")
        for spec in specs
    ):
        return False
...
def _use_lockstep_mla_allocation(
    ...
    """Return whether mixed MLA groups share one block-ID namespace."""
...
def _get_kv_cache_config_packed(
    ...
    parallel_config = vllm_config.parallel_config
    _validate_partial_indexer_kvarn_capacity(
        vllm_config,
        kv_cache_groups,
        minimal_profile=available_memory == 0,
    )
    if _use_lockstep_mla_allocation(
        kv_cache_groups,
        parallel_config.decode_context_parallel_size,
...
def group_and_unify_kv_cache_specs(
    ...
    # Only full MLA/indexer specs participate in lockstep detection; unrelated
    # SWA/state groups cannot hide a valid partial-replication layout.
    ...
    # Keep incompatible custom-CP types, page layouts, and physical shard
    # counts separate. Logical block IDs are synchronized later.
    ...
    for sm_spec in swa_mla_specs:
        ...
        for layer_name, layer_spec in sm_spec.kv_cache_specs.items():
            ...
            candidate = (
                ...
                else size_to_candidate[current_size]
            )
...
def get_kv_cache_configs(
    ...
    # pages plus the separately allocated graph-static workspace fit the
    # profiled per-worker allocation before replacing it with this capacity.
    override = vllm_config.cache_config.num_gpu_blocks_override
    if override is not None:
            ...
            if workspace_config is not None and override > 0:
                ...
                if effective_memory > avail_mem:
                    raise ValueError(
                        ...
                        "pages and the graph-static workspace, but only "
