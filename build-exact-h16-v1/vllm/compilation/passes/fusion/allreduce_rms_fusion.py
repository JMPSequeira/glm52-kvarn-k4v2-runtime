...
def call_b12x_fused_allreduce_add_rms_norm(
    ...
    if custom_allreduce is not None:
        if custom_allreduce.try_fused_add_rms_norm(
            allreduce_in,
            residual,
            rms_gamma,
            rms_eps,
            ...
            return
        # The fused one-shot path is intentionally small-batch-only. Large
        # prefill tensors can still use the same backend's DMA all-reduce.
        allreduce_out = custom_allreduce.custom_all_reduce(allreduce_in)
    else:
        allreduce_out = None
