...
def get_indexer_dcp_group(
    ...
    """Return the process group matching an indexer's physical KV shards."""
    ...
    if dcp_group.world_size == expected_world_size:
        return dcp_group
    partial_size = None if _INDEXER_DCP is None else _INDEXER_DCP.world_size
    raise RuntimeError(
...
def checkpoint_b12x_graph_channels() -> tuple[tuple[Callable[[Any], None], Any], ...]:
    ...
    for group in (_TP, _DCP, _INDEXER_DCP, _INDEXER_QUERY_SPLIT, _PP):
...
def initialize_model_parallel(
    ...
    # At TP4/DCP4 with two shards these are {0,1}/{2,3}; equal shard positions
    # form the query-split groups {0,2}/{1,3}.
    ...
    indexer_shards = parallel_config.dcp_indexer_shards
