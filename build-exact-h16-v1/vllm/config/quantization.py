...
_CHECKPOINT_ONLINE_OVERLAY_NAMES = frozenset(
    {
        "modelopt",
        "modelopt_fp4",
        "modelopt_mxfp8",
...
_CHECKPOINT_ONLINE_OVERLAY_WEIGHTS = frozenset(
...
def _is_checkpoint_online_overlay(
    ...
    if quantization not in _CHECKPOINT_ONLINE_OVERLAY_NAMES or args.moe is not None:
        return False
    ...
    return all(
        spec.weight in _CHECKPOINT_ONLINE_OVERLAY_WEIGHTS
...
def resolve_quantization_config(
    ...
    `quantization_config` take precedence over the shorthand. Supported
    checkpoint formats accept overlays for BF16 dense/shared-expert projections.
    """
    ...
    if quantization is not None and quantization not in ONLINE_QUANT_SHORTHAND_NAMES:
        if quantization_config is not None and not _is_checkpoint_online_overlay(
            ...
            raise ValueError(
                ...
                f"using a supported checkpoint online overlay (weight='mxfp8' or "
