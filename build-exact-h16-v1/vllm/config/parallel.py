...
def _validate_dcp_indexer_shards(indexer_shards: int, dcp_size: int) -> int:
    """Validate the sparse-indexer KV topology and return its shard count."""
    if indexer_shards == 0:
        return dcp_size
    if (
        indexer_shards < 1
        or indexer_shards > dcp_size
        or dcp_size % indexer_shards != 0
    ):
        raise ValueError(
            "VLLM_DCP_INDEXER_SHARDS must be zero or a positive divisor of "
            f"the configured DCP size; got shards={indexer_shards}, DCP={dcp_size}"
        )
    return indexer_shards
...
class ParallelConfig:
    ...
    def _validate_parallel_config(self) -> Self:
        ...
        indexer_shards = _validate_dcp_indexer_shards(
            int(envs.VLLM_DCP_INDEXER_SHARDS),
            self.decode_context_parallel_size,
        )
        if (
            indexer_shards != self.decode_context_parallel_size
            and self.prefill_context_parallel_size != 1
        ):
            raise ValueError("Partial sparse-indexer KV replication requires PCP1")

        return self

    @property
    def dcp_indexer_shards(self) -> int:
        return _validate_dcp_indexer_shards(
            int(envs.VLLM_DCP_INDEXER_SHARDS),
            self.decode_context_parallel_size,
        )
