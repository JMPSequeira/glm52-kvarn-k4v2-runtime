...
class SpeculativeConfig:
    ...
    target model's `--kv-cache-dtype` (the inherit-when-unset convention).
    Accepts the self-describing MLA KVarN strings `kvarn_mla_k{2|4|5}_g{64|128}`;
    when set for an MLA KVarN target it must equal the target's dtype — a
    target/drafter mismatch is a hard config error, not a silent fallback."""
