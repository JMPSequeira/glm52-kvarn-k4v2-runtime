"""CuTeDSL cooperative producer for native KVarN5 sparse-MLA decode."""
...
_LATENT_DIM = 512
_ROPE_DIM = 64
_S_COL_OFFSET = 20_480
_ZP_OFFSET = 21_504
_S_ROW_OFFSET = 22_528

@dsl_user_op
def _ld_global_nc_u8(base_ptr: Int64, *, loc=None, ip=None) -> Uint32:
    """Load one K5 payload byte from an arbitrary global address."""
    return Uint32(
        llvm.inline_asm(
            T.i32(),
...
def io_issue_kvarn_k5_gather(
    ...
    *,
    ...
    ``exact_fp8=False`` reconstructs packed K5 and exact rows as BF16.
    ``exact_fp8=True`` copies exact E4M3 rows and writes unit inline scales.
    ``exact_pool_only=True`` rejects packed K5 without changing the exact source;
    ...
    zero-filled. Map value ``-1`` alone authorizes packed K5 in mixed mode.
    ...
    while meta_entry < Int32(_TILE):
        ...
        if cutlass.const_expr(not exact_fp8 or not exact_fast_io):
            ...
            if packed:
                ...
                row_scale = _load_f16(
                    cache_u8,
                    base + Int64(_S_ROW_OFFSET) + token.to(Int64) * Int64(2),
                )
            st_shared_f32(
    ...
    while entry < Int32(_TILE):
        ...
        if exact:
            ...
            while dim < Int32(_LATENT_DIM):
                ...
                value1 = Float32(0.0)
                if packed:
                    byte_position0 = dim * Int32(40) + token_byte
                    ...
                    dim1 = dim + Int32(1)
                    byte_position1 = dim1 * Int32(40) + token_byte
        ...
        if cutlass.const_expr(exact_fp8 and exact_fast_io):
            ...
            if exact:
                ...
                rope_bits = ld_global_nc_u32(
                    get_ptr_as_int64(rope_pool, rope_offset)
                )
