...
def _convert_req_index_to_global_index_kernel(
    ...
    # A single program owns the whole row and can store its count without atomics.
    ...
    if COMPACT_TO_FRONT:
        ...
        # sum gives each valid lane a distinct local offset. With one program per
        # row, the prefix remains in input-column order and needs no atomic.
...
def _remap_tiling(
    ...
    """Choose deterministic one-program rows for the opt-in DCP compact path."""
    single_tile = (
        _DETERMINISTIC_DCP_COMPACT
        and count_valid
...
def triton_convert_req_index_to_global_index(
    ...
    # Zero initialization is only required by the multi-tile atomic path.
...
def triton_filter_and_convert_dcp_index(
    ...
    if out is None:
        ...
        if compact_valid_to_front and not out_preinitialized:
            out.fill_(-1)
    ...
    if count_valid:
        if valid_counts is None:
            ...
            if not single_tile and not out_preinitialized:
                valid_counts.zero_()
    else:
        valid_counts = None
