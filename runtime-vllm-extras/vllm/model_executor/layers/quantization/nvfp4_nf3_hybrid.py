...
# VLLM_B12X_MOE_DECODE_M can raise this ceiling at launch-compile time (e.g.
# 9 so DSpark K8 verify batches ride the direct-top-k decode launch instead
# of the packed prefill launch); 0/unset keeps the pinned default below.
_B12X_DECODE_M = 8
...
class _HybridSharedRuntime:
    ...
    def __init__(self) -> None:
        ...
        # Token ceiling for the TC-decode launch. Resolved once from
        # VLLM_B12X_MOE_DECODE_M (0/unset -> pinned _B12X_DECODE_M) because
        # envs attribute access hits os.environ and apply() dispatches on it.
        override = int(envs.VLLM_B12X_MOE_DECODE_M)
        self.decode_m: int = override if override > 0 else _B12X_DECODE_M
...
class NvFp4Nf3HybridMoEMethod(FusedMoEMethodBase):
    ...
    def _get_launch_pair(self, prepared: Any) -> tuple[Any, Any]:
        ...
        decode = prefill
        decode_m = min(runtime.decode_m, int(runtime.max_m))
        try:
            candidate = compile_w4a16_fused_moe(
                size_m=decode_m,
                zero_fc2_output=False,
                moe_block_size=8,
                max_m_blocks=decode_m * runtime.topk,
