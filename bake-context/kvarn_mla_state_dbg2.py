...
class KVarNMLAStateManager:
    ...
    def prepare_step(
        ...
        if os.path.exists('/tmp/KVARN_DBG_APC'):
            cls._dbg_steps = getattr(cls, '_dbg_steps', 0) + 1
            if cls._dbg_steps <= 4000:
                logger.warning(
                    'APCDBG2 pstep step=%d q=%d needed=%s retired=%s flush=%s '
                    'assign=%s free=%d tail=%s',
                    cls._dbg_steps,
                    getattr(common_metadata, 'num_actual_tokens', -1),
                    sorted(block_fills.items()),
                    [(b, state.block_fill.get(b, 0)) for b in retired],
                    flush_ids,
                    [(b, state.mapping[b]) for b in missing],
