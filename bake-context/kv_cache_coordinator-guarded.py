...
def get_kv_cache_coordinator(
    ...
    # Single-group MLA/KVarN layouts are chunk-partition non-invariant under
    # DCP: prefill logits depend on batch partitioning, so a full-prefix
    # cache hit (a re-partition) diverges from the fresh forward (measured
    # 2026-08-18: token-0 logprob delta 5.2e-2; identical greedy prompts
    # diverge with cache fully off across max_num_batched_tokens 2048 vs
    # 4096). Disable prefix caching for these layouts under DCP>1 until
    # prefill is shape-invariant.
    if (
        enable_caching
        and dcp_world_size > 1
        and isinstance(
