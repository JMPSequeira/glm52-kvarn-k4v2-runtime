...
if TYPE_CHECKING:
    ...
    VLLM_KVARN_MLA_EXACT_CUDAGRAPH: bool = False
    ...
    VLLM_DSPARK_NGRAM_ASSIST: bool = False
    VLLM_DSPARK_NGRAM_K: int = 5
    VLLM_DSPARK_NGRAM_MAX_BATCH: int = 64
    VLLM_DSPARK_NGRAM_MAX_INDEX_TOKENS: int = 65536
    VLLM_DSPARK_NGRAM_ASSIST_LOG: str = ""
    VLLM_MTP_TABLE_EXTENSION: int = 0
    VLLM_MTP_TABLE_K: int = 1
    VLLM_MTP_TABLE_LOG: str = ""
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Capture and dispatch separate native KVarN MLA exact-only CUDA graphs.
    # Default off for controlled performance A/B; exact side-pool ownership
    # and mixed K5 graph behavior are unchanged.
    "VLLM_KVARN_MLA_EXACT_CUDAGRAPH": lambda: bool(
        int(os.getenv("VLLM_KVARN_MLA_EXACT_CUDAGRAPH", "0"))
    ),
    ...
    # Number of unique sparse-indexer KV shards inside the configured DCP
    # group. Zero keeps the configured DCP size. Intermediate divisors create
    # replicated shard groups; for example, two under DCP4 creates two 2-rank
    # replicas. The feature is default-off.
    "VLLM_DCP_INDEXER_SHARDS": lambda: int(
        os.getenv("VLLM_DCP_INDEXER_SHARDS", "0")
    ),
    ...
    # DSpark n-gram assist (default off): host-side per-slot override of
    # the draft proposal from a rightmost-occurrence k-gram index over
    # each request's prompt + accepted-token stream. The captured draft
    # graph is untouched and greedy verification is unchanged, so served
    # outputs are identical; only acceptance length moves. VLLM_DSPARK_NGRAM_K
    # is the exact-match context length (offline-promoted k=5). The assist
    # is skipped (fail-closed to the unassisted drafter) for batches larger
    # than VLLM_DSPARK_NGRAM_MAX_BATCH or requests whose index exceeds
    # VLLM_DSPARK_NGRAM_MAX_INDEX_TOKENS grams (bounded most-recent window).
    # VLLM_DSPARK_NGRAM_ASSIST_LOG enables per-iteration JSONL decision
    # logging (iteration, slot, fired, k, changed).
    "VLLM_DSPARK_NGRAM_ASSIST": lambda: bool(
        int(os.getenv("VLLM_DSPARK_NGRAM_ASSIST", "0"))
    ),
    "VLLM_DSPARK_NGRAM_K": lambda: int(os.getenv("VLLM_DSPARK_NGRAM_K", "5")),
    "VLLM_DSPARK_NGRAM_MAX_BATCH": lambda: int(
        os.getenv("VLLM_DSPARK_NGRAM_MAX_BATCH", "64")
    ),
    "VLLM_DSPARK_NGRAM_MAX_INDEX_TOKENS": lambda: int(
        os.getenv("VLLM_DSPARK_NGRAM_MAX_INDEX_TOKENS", "65536")
    ),
    "VLLM_DSPARK_NGRAM_ASSIST_LOG": lambda: os.getenv(
        "VLLM_DSPARK_NGRAM_ASSIST_LOG", ""
    ),
    "VLLM_DSPARK_NGRAM_DEBUG": lambda: int(
        os.getenv("VLLM_DSPARK_NGRAM_DEBUG", "0")
    ),
    # MTP table extension (default off): extend the MTP draft block past
    # the head's steps with tokens from a per-request online n-gram table
    # (chain-mode keying, per-slot re-query, top-1 continuation). Configure
    # the engine with num_speculative_tokens = head_steps + extension so
    # scheduler padding / verify rows / CUDA graph capture sizes stay
    # static; misses fail closed to declined (-1) extension slots, i.e.
    # the unextended arm. Incompatible with the FixedMTP3 supergraph.
    "VLLM_MTP_TABLE_EXTENSION": lambda: int(
        os.getenv("VLLM_MTP_TABLE_EXTENSION", "0")
    ),
    # N-gram order for the table key. 1 (default) is chain-mode: the key
    # is the chain's own last accepted token.
    "VLLM_MTP_TABLE_K": lambda: int(os.getenv("VLLM_MTP_TABLE_K", "1")),
    # Optional JSONL per-step decision log (fired/declined per slot).
    "VLLM_MTP_TABLE_LOG": lambda: os.getenv("VLLM_MTP_TABLE_LOG", ""),
