...
def _dflash_layer_causal(config: Qwen3Config, layer_idx: int) -> bool:
    """Resolve draft causality from the Speculators checkpoint."""
    override = (getattr(config, "dflash_config", None) or {}).get("causal")
    if override is not None:
        return override
    layer_types = getattr(config, "layer_types", None)
    is_sliding = bool(layer_types) and layer_types[layer_idx] == _SLIDING_ATTENTION
    return is_sliding and not bool(
        getattr(config, "sliding_window_non_causal", False)
    )
...
class DFlashAttention(Attention):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec | None:
        ...
        context_sliding_window = getattr(
            self, "context_sliding_window", self.sliding_window
        )
        if context_sliding_window is not None:
...
class DFlashQwen3Attention(nn.Module):
    ...
    def __init__(
        ...
        context_sliding_window: int | None = None,
        causal: bool = False,
        ...
        self.sliding_window = context_sliding_window
        ...
        self.attn.context_sliding_window = context_sliding_window
        self.causal = causal
    ...
    def __init__(
        ...
        *,
        ...
        if speculative_config is not None and speculative_config.use_dspark():
            causal = sliding_window is not None and not bool(
                getattr(config, "sliding_window_non_causal", False)
            )
        attention_window = sliding_window
        dspark_anchor_window = None
        if (
            causal
            ...
            dspark_anchor_window = sliding_window
        ...
        self.self_attn = DFlashQwen3Attention(
            ...
            causal=causal,
            context_sliding_window=sliding_window,
        ...
        self.self_attn.attn.dspark_anchor_window = dspark_anchor_window
        self.mlp = Qwen3MLP(
            hidden_size=self.hidden_size,
...
class DFlashQwen3Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.use_aux_hidden_state:
            ...
            elif hasattr(self.config, "target_layer_ids"):
    ...
    def _project_context_kv(
        ...
        # The context is already normalized by combine_hidden_states.
