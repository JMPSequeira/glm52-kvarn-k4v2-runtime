    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if is_kvarn:
            if cache_config.kv_offloading_size is not None:
                raise ValueError("KVarN does not support KV cache offloading.")
            ...
            if (
                ...
                raise ValueError(
                    "KVarN does not support KV transfer or disaggregated serving."
                )
            ...
            if is_kvarn_mla_cache_dtype(cache_dtype):
                if not model_config.use_mla:
                    raise ValueError(f"{cache_dtype} requires an MLA model.")
                ...
                if spec_config is not None and getattr(
                    ...
                    spec_kv_cache_dtype = getattr(
                        spec_config, "kv_cache_dtype", None
                    )
                    if spec_kv_cache_dtype is not None and (
                        spec_kv_cache_dtype != cache_dtype
                    ):
                        raise ValueError(
                            "MLA KVarN speculative decoding requires the "
                            "drafter kv_cache_dtype to equal the target "
                            f"--kv-cache-dtype ({cache_dtype!r}); got "
                            f"{spec_kv_cache_dtype!r}. Omit the drafter "
                            "kv_cache_dtype to inherit the target dtype."
                        )
                kvarn_mla_group = KVarNMLAConfig.from_cache_dtype(
                    cache_dtype
                ).group
                if cache_config.block_size != kvarn_mla_group:
                    raise ValueError(
                        f"{cache_dtype} requires --block-size "
                        f"{kvarn_mla_group}; got {cache_config.block_size}."
                    )
                if parallel_config.enable_dbo:
                    raise ValueError("MLA KVarN does not support dual-batch overlap.")
                ...
                if model_config.use_mla:
                    raise ValueError(
                        "MLA KVarN requires a self-describing kv_cache_dtype "
