...
from vllm.v1.worker.gpu.spec_decode.rejection_sampler_utils import (
    greedy_rejection_sample_from_argmax,
    rejection_sample,
)
...
class RejectionSampler:
    ...
    def can_use_greedy_token_ids(self, input_batch: InputBatch) -> bool:
        idx_mapping_np = input_batch.idx_mapping_np
        sampler = self.sampler
        return (
            self.synthetic_conditional_rates is None
            and not self.use_block_verification
            and not sampler.compute_nans
            and (
                sampler.sampling_states.temperature.np[idx_mapping_np] == 0.0
            ).all()
            and sampler.sampling_states.max_num_logprobs(idx_mapping_np)
            == NO_LOGPROBS
            and sampler.logprob_token_ids_state.max_num_token_ids(idx_mapping_np) == 0
            and not sampler.logit_bias_state.use_logit_bias[idx_mapping_np].any()
            and not sampler.penalties_state.use_penalty[idx_mapping_np].any()
            and sampler.bad_words_state.num_bad_words.np[idx_mapping_np].max() == 0
        )

    def from_greedy_token_ids(
        self,
        ...
        sampled, num_sampled = greedy_rejection_sample_from_argmax(
            target_token_ids,
            draft_sampled,
            input_batch.cu_num_logits,
