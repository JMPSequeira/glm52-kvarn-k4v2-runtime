...
def _greedy_rejection_kernel(
    ...
    # Accept an exact greedy prefix and emit its first mismatch or bonus.
    req_idx = tl.program_id(0)
    output_offsets = tl.arange(0, OUTPUT_BLOCK_SIZE)
    tl.store(
        sampled_ptr + req_idx * sampled_stride + output_offsets,
        0,
    ...
    accepted = True
    output_length = tl.zeros((), tl.int32)
    for i in range(num_draft_tokens):
        if accepted:
            target_token = tl.load(
                target_argmax_ptr + start_idx + i
            ).to(tl.int64)
            draft_token = tl.load(
                draft_sampled_ptr + start_idx + i + 1
            ).to(tl.int64)
            tl.store(sampled_ptr + req_idx * sampled_stride + i, target_token)
            output_length += 1
            accepted &= draft_token == target_token

    if accepted:
        bonus_token = tl.load(target_argmax_ptr + end_idx - 1).to(tl.int64)
        tl.store(
    ...
    tl.store(num_sampled_ptr + req_idx, output_length)


def greedy_rejection_sample_from_argmax(
    ...
    _greedy_rejection_kernel[(num_reqs,)](
        sampled,
        sampled.stride(0),
        num_sampled,
        target_argmax,
        draft_sampled,
        cu_num_logits,
        OUTPUT_WIDTH=num_speculative_steps + 1,
        OUTPUT_BLOCK_SIZE=triton.next_power_of_2(num_speculative_steps + 1),
        num_warps=1,
    )
...
def _greedy_rejection_sample(
    ...
    """Exact counterpart of ``vllm.v1.sample`` greedy rejection sampling."""
    # Match the general path's effective vocabulary when the draft vocabulary
    # is narrower than the target vocabulary.
    target_argmax = target_logits[:, :vocab_size].argmax(dim=-1)
    return greedy_rejection_sample_from_argmax(
        target_argmax,
        draft_sampled,
        cu_num_logits,
        num_speculative_steps,
...
def rejection_sample(
    ...
    # Exact greedy verification needs neither probability statistics nor
    # recovery sampling. Keep synthetic acceptance on the general path because
    # it intentionally rejects matching drafts according to its configured RNG.
    if all_greedy and synthetic_conditional_rates is None:
