...
def _should_share(eagle: nn.Module, flag: str, draft, target) -> bool:
    ...
    if draft_weight is None or target_weight is None:
        return False
    if (
        draft_weight.is_cuda
        and torch.accelerator.get_memory_info(draft_weight.device)[0]
        < draft_weight.numel() * 2
    ):
        return torch.equal(draft_weight.cpu(), target_weight.cpu())
    return torch.equal(draft_weight, target_weight)
