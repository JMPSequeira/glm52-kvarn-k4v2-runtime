...
_GLM_STAGE_HASH_ENABLED = os.getenv("VLLM_GLM_STAGE_HASH", "0") == "1"
_GLM_STAGE_HASH_LAYERS = frozenset(range(6))
_GLM_STAGE_HASH_SLOTS = (
    "entry_hidden",
    "entry_residual",
...
def _glm_stage_hash_startup_marker() -> str:
    slots = ",".join(_GLM_STAGE_HASH_SLOTS)
    return (
        "GLM_STAGE_HASH_OVERLAY_ACTIVE "
        "class=GlmMoeDsaForCausalLM "
        "module=vllm.model_executor.models.deepseek_v2 "
        f"enabled={int(_GLM_STAGE_HASH_ENABLED)} layers=0,1,2,3,4,5 "
        f"slots={slots} hash=aten.hash_tensor(mode0,rowwise) "
        "buffer=6x8x9+embedding9 dynamic_rows=slice9 "
        "sequence_parallel=disabled logical_m9=runner "
        "generation=post_mlp_int64"
    )
...
class DeepseekV2MLP(nn.Module):
    def __init__(
        ...
        is_shared_expert = prefix.endswith(".shared_experts")
        self.exl3_fused_mlp = (
            Exl3FusedMlpMethod(
                quant_config,
                prefix,
            ...
            and (is_shared_expert or prefix.endswith(".mlp"))
            else None
        )

    def forward(self, x):
        if self.exl3_fused_mlp is not None:
            output = self.exl3_fused_mlp.apply(self.gate_up_proj, self.down_proj, x)
...
class Indexer(nn.Module):
    def __init__(
        ...
        if isinstance(quant_config, Exl3Config):
            # EXL3 tensor metadata and parameters are keyed by the checkpoint
            # source prefixes, so WK must remain independent from the
            # unquantized weights projection.
            ...
            # Fused wk + weights_proj: single GEMM producing
            # [head_dim + n_head]. FP8 wk weights are upcasted to BF16 during
            # loading to maintain fusion.
    ...
    def _project_kw(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if isinstance(self.quant_config, Exl3Config):
            k, _ = self.wk(hidden_states)
            weights, _ = self.weights_proj(hidden_states)
            return torch.cat((k, weights), dim=-1)
    ...
    def forward(
        ...
        if kw is None:
            kw = self._project_kw(hidden_states)
        k = kw[:, : self.head_dim]
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(
        ...
        self.routed_scaling_factor = getattr(config, "routed_scaling_factor", 1.0)
        self._glm_stage_hash_enabled = (
            _GLM_STAGE_HASH_ENABLED and layer_idx in _GLM_STAGE_HASH_LAYERS
        )
        if self._glm_stage_hash_enabled:
            self._glm_stage_hash_tp_world = get_tensor_model_parallel_world_size()
            if self._glm_stage_hash_tp_world != 4:
                raise RuntimeError("VLLM_GLM_STAGE_HASH requires TP4")
            if self.use_sequence_parallel_moe:
                raise RuntimeError(
                    "VLLM_GLM_STAGE_HASH requires non-sequence-parallel layers 0-5"
                )
            self.register_buffer(
                "_glm_stage_hashes",
                torch.zeros(
                    (len(_GLM_STAGE_HASH_SLOTS), 9),
                    dtype=torch.uint64,
            ...
            self.register_buffer(
                "_glm_stage_hash_generation",
                torch.zeros((), dtype=torch.int64, device=current_platform.device_type),
                persistent=False,
            )

    def _record_glm_stage_hash(
        self,
        slot: int,
        tensor: torch.Tensor | None,
    ) -> None:
        if self._glm_stage_hash_enabled and tensor is not None:
            if slot == 0:
                self._glm_stage_hashes.zero_()
            source = tensor.detach()[:9]
            row_hashes = torch.hash_tensor(source, dim=-1)
            staged_hashes = torch.nn.functional.pad(row_hashes, (0, 9))[:9]
            self._glm_stage_hashes[slot].copy_(staged_hashes)
            if slot == len(_GLM_STAGE_HASH_SLOTS) - 1:
                self._glm_stage_hash_generation.add_(1)
    ...
    def forward(
        ...
        self._record_glm_stage_hash(0, hidden_states)
        self._record_glm_stage_hash(1, residual)
        ...
        self._record_glm_stage_hash(2, hidden_states)
        self._record_glm_stage_hash(3, residual)
        ...
        self._record_glm_stage_hash(4, hidden_states)
        ...
        self._record_glm_stage_hash(5, hidden_states)
        self._record_glm_stage_hash(6, residual)
        if self.use_sequence_parallel_moe:
        ...
        self._record_glm_stage_hash(7, hidden_states)
...
class DeepseekV2Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        logger.warning_once("%s", _glm_stage_hash_startup_marker())
        ...
        if _GLM_STAGE_HASH_ENABLED:
            self.register_buffer(
                "_glm_input_embedding_hashes",
                torch.zeros(
                    9,
                    dtype=torch.uint64,
                    device=current_platform.device_type,
    ...
    def _record_glm_input_embedding_hash(
        self, hidden_states: torch.Tensor
    ) -> None:
        if _GLM_STAGE_HASH_ENABLED:
            source = hidden_states.detach()[:9]
            row_hashes = torch.hash_tensor(source, dim=-1)
            staged_hashes = torch.nn.functional.pad(row_hashes, (0, 9))[:9]
            self._glm_input_embedding_hashes.copy_(staged_hashes)
    ...
    def forward(
        ...
        if get_pp_group().is_first_rank:
            ...
            self._record_glm_input_embedding_hash(hidden_states)
            residual = None
        else:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        is_exl3 = isinstance(self.quant_config, Exl3Config)
        if not is_exl3:
            # Fused indexer wk + weights_proj
            # (shard 0 = wk, shard 1 = weights_proj).
        ...
        for name, loaded_weight in weights:
            ...
            if not is_exl3 and _try_load_fp8_indexer_wk(
                name,
                loaded_weight,
                _pending_wk_fp8,
                params_dict,
...
class DeepseekV2ForCausalLM(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.fuse_qkv_a_proj = (
            ...
            and not isinstance(quant_config, Exl3Config)
        )
        ...
        if get_pp_group().is_last_rank:
            lm_head_kwargs = {}
            if isinstance(quant_config, Exl3Config):
                # EXL3 applies its output Hadamard transform in independent
                # 128-row blocks.  Padding by the full TP topology keeps every
                # rank's packed output slice on a block boundary.
                lm_head_kwargs["padding_size"] = (
                    get_tensor_model_parallel_world_size() * 128
                )
