...
_GLM_STAGE_HASH_DRAIN_ENABLED = os.getenv("VLLM_GLM_STAGE_HASH", "0") == "1"
_FULL_GRAPH_REQUEST_STATE_GUARD = (
    os.getenv("VLLM_FULL_GRAPH_REQUEST_STATE_GUARD", "0") == "1"
)
_GLM_STAGE_HASH_SLOT_NAMES = (
    "entry_hidden",
    "entry_residual",
    "post_input_norm_hidden",
    "post_input_norm_residual",
    "post_attention_hidden",
    "post_attention_norm_hidden",
    "post_attention_norm_residual",
    "post_mlp_hidden",
)


def _guard_full_graph_request_state(
    uniform_tok_count: int | None,
    request_ids: tuple[str, ...],
    req_id_to_index: dict[str, int],
    num_computed_prefill_tokens: np.ndarray,
    prefill_lengths: np.ndarray,
) -> tuple[int | None, tuple[str, ...]]:
    prefilling = tuple(
        req_id
        for req_id in request_ids
        if num_computed_prefill_tokens[req_id_to_index[req_id]]
        < prefill_lengths[req_id_to_index[req_id]]
    )
    if prefilling:
...
def _resolve_glm_stage_hash_layers(model: nn.Module) -> tuple[nn.Module, list[nn.Module]]:
    target_model = getattr(model, "model", None)
    layers = getattr(target_model, "layers", None)
    if layers is None or len(layers) <= 5:
        raise RuntimeError(
            "VLLM_GLM_STAGE_HASH requires GlmMoeDsaForCausalLM.model.layers[0:6]"
        )
    selected_layers = [layers[layer_idx] for layer_idx in range(6)]
    for layer_idx, layer in enumerate(selected_layers):
        hashes = getattr(layer, "_glm_stage_hashes", None)
        generation = getattr(layer, "_glm_stage_hash_generation", None)
        if (
            not isinstance(hashes, torch.Tensor)
            or tuple(hashes.shape) != (8, 9)
            or hashes.dtype != torch.uint64
            or not isinstance(generation, torch.Tensor)
            or generation.dtype != torch.int64
            or generation.numel() != 1
        ):
            raise RuntimeError(
                f"VLLM_GLM_STAGE_HASH layer {layer_idx} lacks the active 8x9 seam"
            )
    embedding_hashes = getattr(target_model, "_glm_input_embedding_hashes", None)
    if (
        not isinstance(embedding_hashes, torch.Tensor)
        or tuple(embedding_hashes.shape) != (9,)
        or embedding_hashes.dtype != torch.uint64
    ):
        raise RuntimeError(
            "VLLM_GLM_STAGE_HASH target lacks the active embedding9 seam"
        )
    return target_model, selected_layers


def _read_glm_stage_hash_generation(selected_layers: list[nn.Module]) -> int:
    generations = [
        int(layer._glm_stage_hash_generation.item()) for layer in selected_layers
    ]
    if len(set(generations)) != 1:
        raise RuntimeError(f"GLM stage-hash generation mismatch: {generations}")
    return generations[0]


def _prepare_glm_stage_hash_matrix(model: nn.Module) -> int:
    _, selected_layers = _resolve_glm_stage_hash_layers(model)
    torch.cuda.synchronize()
    return _read_glm_stage_hash_generation(selected_layers)


def _glm_tensor_content(tensor: torch.Tensor, *, name: str) -> list[Any]:
    if tensor.numel() == 0:
        raise RuntimeError(f"VLLM_GLM_STAGE_HASH {name} tensor is empty")
    return tensor.detach().reshape(-1).cpu().tolist()


def _glm_ordered_content_hash(tensor: torch.Tensor, *, name: str) -> str:
    if tensor.numel() == 0:
        raise RuntimeError(f"VLLM_GLM_STAGE_HASH {name} tensor is empty")
    source = tensor.detach().cpu().contiguous()
    digest = hashlib.sha256()
    digest.update(str(source.dtype).encode())
    digest.update(str(tuple(source.shape)).encode())
    digest.update(source.numpy().tobytes())
    return digest.hexdigest()


def _resolve_glm_graph_metadata(attn_metadata: dict[str, Any]) -> Any:
    required = (
        "query_start_loc",
        "slot_mapping",
        "block_table",
        "seq_lens",
        "cache_seq_lens_per_req",
        "cache_seq_lens_per_token",
    )
    for metadata in attn_metadata.values():
        if all(hasattr(metadata, field) for field in required):
            return metadata
    raise RuntimeError(
        "VLLM_GLM_STAGE_HASH requires B12X graph-visible attention metadata"
    )


def _snapshot_glm_runner_inputs(
    *,
    ...
    if input_ids.numel() < 9 or positions.numel() < 9:
        raise RuntimeError(
            "VLLM_GLM_STAGE_HASH M9 input backing tensors have fewer than 9 values"
        )
    metadata = _resolve_glm_graph_metadata(attn_metadata)
    req_id_per_token = getattr(metadata, "req_id_per_token", None)
    if not isinstance(req_id_per_token, torch.Tensor):
        raise RuntimeError(
            "VLLM_GLM_STAGE_HASH TP4/DCP4 metadata lacks req_id_per_token"
        )
    active_cta_lookup = getattr(metadata, "active_cta_lookup", None)
    return {
        "input_ids": _glm_tensor_content(input_ids.reshape(-1)[:9], name="input_ids"),
        "positions": _glm_tensor_content(
            positions.reshape(-1)[:9], name="positions"
        ),
        "query_start_loc": _glm_tensor_content(
            metadata.query_start_loc.reshape(-1)[:2], name="query_start_loc"
        ),
        "slot_mapping": _glm_tensor_content(
            metadata.slot_mapping.reshape(-1)[:9], name="slot_mapping"
        ),
        "block_table": _glm_ordered_content_hash(
            metadata.block_table[:1], name="block_table"
        ),
        "seq_lens": _glm_tensor_content(
            metadata.seq_lens.reshape(-1)[:1], name="seq_lens"
        ),
        "cache_seq_lens_per_req": _glm_tensor_content(
            metadata.cache_seq_lens_per_req.reshape(-1)[:1],
            name="cache_seq_lens_per_req",
        ),
        "req_id_per_token": _glm_tensor_content(
            req_id_per_token.reshape(-1)[:9], name="req_id_per_token"
        ),
        "cache_seq_lens_per_token": _glm_tensor_content(
            metadata.cache_seq_lens_per_token.reshape(-1)[:9],
            name="cache_seq_lens_per_token",
        ),
        "active_cta_lookup": (
            _glm_tensor_content(
                active_cta_lookup.reshape(-1)[:9], name="active_cta_lookup"
            )
            if isinstance(active_cta_lookup, torch.Tensor)
            else None
        ),
        "is_padding": _glm_tensor_content(
            input_batch.is_padding.reshape(-1)[:9], name="is_padding"
        ),
        "scalars": {
            ...
            "prefill_max_seq_len": int(metadata.prefill_max_seq_len),
            "is_spec_decode": bool(metadata.is_spec_decode),
            "causal": getattr(metadata, "causal", None),
            "dcp_local_total_tokens": int(metadata.dcp_local_total_tokens),
            "dcp_padded_total_tokens": int(metadata.dcp_padded_total_tokens),
            "dcp_ckv_gather_eligible": bool(metadata.dcp_ckv_gather_eligible),
            "active_cta_plan_id": getattr(metadata, "active_cta_plan_id", None),
            "active_cta_grid_ctas": getattr(metadata, "active_cta_grid_ctas", None),
            "block_size": int(metadata.block_size),
            "topk_tokens": int(metadata.topk_tokens),
        },
    }


def _drain_glm_stage_hash_matrix(
    model: nn.Module,
    ...
    *,
    ...
    target_model, selected_layers = _resolve_glm_stage_hash_layers(model)
    torch.cuda.synchronize()
    post_generations = [
        int(layer._glm_stage_hash_generation.item()) for layer in selected_layers
    ]
    if len(set(post_generations)) != 1:
        raise RuntimeError(
            f"GLM stage-hash post-generation mismatch: {post_generations}"
        )
    post_generation = post_generations[0]
    if post_generation != pre_generation + 1:
        raise RuntimeError(
            "GLM_STAGE_HASH_EMISSION_SELF_CHECK_FAILED: "
            f"cg_mode={cg_mode.name} positions={position_values} "
            f"model_class={type(model).__name__} "
            f"target_class={type(target_model).__name__} "
            f"pre_generation={pre_generation} "
            f"post_generations={post_generations}"
        )
    if len(position_values) != 9:
        raise RuntimeError(
            "GLM stage-hash M9 position snapshot does not contain 9 values: "
            f"positions={position_values}"
        )
    if len(request_ids) != 1:
        raise RuntimeError(
            f"GLM stage-hash diagnostic requires C1, got requests={request_ids}"
        )
    if tuple(runner_inputs.get("positions", ())) != tuple(position_values):
        raise RuntimeError(
            "GLM stage-hash runner position snapshot is internally inconsistent"
        )
    payload = {
        "rank": (
        ...
        "cg_mode": cg_mode.name,
        "request_ids": request_ids,
        "positions": position_values,
        "runner_inputs": runner_inputs,
        "embedding_hidden": target_model._glm_input_embedding_hashes.detach()
        .cpu()
        .tolist(),
        "slots": _GLM_STAGE_HASH_SLOT_NAMES,
        "layers": {
            str(layer_idx): layer._glm_stage_hashes.detach().cpu().tolist()
            for layer_idx, layer in enumerate(selected_layers)
        },
    }
    logger.warning("GLM_STAGE_HASH_MATRIX %s", json.dumps(payload, separators=(",", ":")))
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self._large_prefill_drain_pending = False
    ...
    def _reset_kv_cache_binding_state(self) -> None:
        ...
        self._kvarn_mla_cache_generation += 1
        ...
        self._kvarn_mla_target_block_fills = None
        if isinstance(self.speculator, DraftModelSpeculator):
            self.speculator.set_kvarn_mla_block_fills(None)
    ...
    def _init_minimal_kv_cache_for_profiling(self) -> None:
        ...
        try:
            minimal_config = get_kv_cache_config_from_groups(
                ...
                kv_cache_groups,
                available_memory=0,
                validate_override_memory=False,
            )
        finally:
    ...
    def _maybe_drain_large_prefill_cache(
        self, scheduler_output: SchedulerOutput
    ) -> None:
        if not self._large_prefill_drain_pending or self.execute_model_state is not None:
            return
        ...
        self._large_prefill_drain_pending = False
        threshold_bytes = 512 << 20
        reclaimable_bytes = torch.cuda.memory_reserved(
            self.device
        ) - torch.cuda.memory_allocated(self.device)
        if reclaimable_bytes < threshold_bytes:
            return
        ...
        logger.info(
            "Released %.1f MiB of large-prefill allocator cache in %.1f ms.",
            (free_after - free_before) / (1 << 20),
    ...
    def _make_kvarn_mla_resolution_callback(
        ...
        def resolve(
            ...
            if cache_generation != self._kvarn_mla_cache_generation:
                return
            for snapshot, num_rejected in zip(
                snapshots, num_rejected_tokens
            ):
                ...
                for tracker in self._kvarn_mla_live_block_trackers.values():
                    tracker.resolve_async(
                        tracker_key, request, actual_end_tokens
                    )
    ...
    def sample(
        ...
        sample_hidden_states = hidden_states[input_batch.logits_indices]
        use_greedy_token_ids = (
            os.getenv("VLLM_GREEDY_LOCAL_ARGMAX", "0") == "1"
            and grammar_output is None
            and input_batch.num_draft_tokens > 0
            and self.rejection_sampler is not None
            and self.rejection_sampler.can_use_greedy_token_ids(input_batch)
        ...
        with record_function_or_nullcontext(f"vllm:v2/target/{phase}/logits"):
            if use_greedy_token_ids:
                logger.info_once(
                    "Using vocab-parallel local argmax for greedy target verification."
                )
                target_token_ids = self.model.logits_processor.get_top_tokens(
                    self.model.lm_head,
        ...
        if input_batch.num_draft_tokens == 0 or self.rejection_sampler is None:
            ...
            with record_function_or_nullcontext(
                ...
                if use_greedy_token_ids:
                    ...
                    sampler_output = self.rejection_sampler.from_greedy_token_ids(
                        target_token_ids,
                        input_batch,
                    )
                else:
    ...
    def execute_model(
        ...
        if not dummy_run:
            with record_function_or_nullcontext(
                "vllm:v2/target/resolve_async_ownership"
            ):
            ...
            self._maybe_drain_large_prefill_cache(scheduler_output)
        ...
        full_graph_state_collision: (
            tuple[tuple[str, ...], int, int, int] | None
        ) = None
        if _FULL_GRAPH_REQUEST_STATE_GUARD and not dummy_run:
            guarded_uniform_tok_count, prefilling_request_ids = (
                _guard_full_graph_request_state(
                    uniform_tok_count,
            ...
            if prefilling_request_ids and uniform_tok_count is not None:
                full_graph_state_collision = (
                    prefilling_request_ids,
                    int(num_toks),
                    int(max_query_len),
                    int(uniform_tok_count),
                )
            uniform_tok_count = guarded_uniform_tok_count
        use_varlen_capacity = (
            verification_capacity_manager is not None
            and verification_capacity_manager.varlen_spec_decode
        ...
        if full_graph_state_collision is not None:
            collision_ids, collision_tokens, collision_width, collision_uniform = (
                full_graph_state_collision
            )
            logger.warning(
                "FULL_GRAPH_REQUEST_STATE_COLLISION "
                "request_ids=%s physical_tokens=%d max_query_len=%d "
                "shape_uniform=%d dispatched_cg=%s",
                collision_ids,
                collision_tokens,
                collision_width,
                collision_uniform,
        ...
        glm_stage_hash_pre_generation: int | None = None
        glm_runner_input_snapshot: dict[str, Any] | None = None
        glm_position_values: list[int] | None = None
        if (
            _GLM_STAGE_HASH_DRAIN_ENABLED
            and not dummy_run
            and int(num_toks) == 9
        ):
            glm_stage_hash_pre_generation = _prepare_glm_stage_hash_matrix(self.model)
            input_ids = model_inputs.get("input_ids")
            positions = model_inputs.get("positions")
            if not isinstance(input_ids, torch.Tensor):
                raise RuntimeError("VLLM_GLM_STAGE_HASH requires target input_ids")
            if not isinstance(positions, torch.Tensor):
                raise RuntimeError("VLLM_GLM_STAGE_HASH requires target positions")
            if not isinstance(attn_metadata, dict):
                raise RuntimeError(
                    "VLLM_GLM_STAGE_HASH requires target attention metadata"
                )
            glm_runner_input_snapshot = _snapshot_glm_runner_inputs(
                input_ids=input_ids,
                positions=positions,
                input_batch=input_batch,
                attn_metadata=attn_metadata,
            )
            glm_position_values = list(glm_runner_input_snapshot["positions"])
        ...
        if (
            ...
            if (
                glm_stage_hash_pre_generation is None
                or glm_runner_input_snapshot is None
                or glm_position_values is None
            ):
                raise RuntimeError(
                    "GLM stage-hash M9 pre-forward snapshot was not captured"
                )
            _drain_glm_stage_hash_matrix(
                self.model,
                glm_position_values,
                glm_runner_input_snapshot,
                cg_mode=batch_desc.cg_mode,
                pre_generation=glm_stage_hash_pre_generation,
        ...
        if (
            ...
            and input_batch.num_tokens == self.max_num_tokens
            and np.any(input_batch.is_prefilling_np)
        ):
            self._large_prefill_drain_pending = True
    ...
    def sample_tokens(
        ...
        phase = _profile_batch_phase(input_batch)
        with record_function_or_nullcontext(
            f"vllm:v2/target/{phase}/sample_tokens"
        ):
        ...
        if ownership_callback is not None:
            if self._kvarn_mla_pending_resolution is not None:
                raise RuntimeError(
                    "MLA KVarN async ownership settlement was overwritten"
                )
